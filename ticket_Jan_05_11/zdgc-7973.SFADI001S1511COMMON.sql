...
CREATE OR REPLACE FUNCTION sfadi001s1511common ( 
    l_inKkSakuseiDt varchar(20), 
    l_inDenbunMeisaiNo numeric, 
    l_inKkStat char(2) ) RETURNS integer AS $body$
DECLARE
...
-- *====================================================================
    ...
    gSql            varchar(400);          -- Dynamic SQL
    sqlCount        numeric;               -- 更新件数
--====================================================================*
--					定数定義
-- *====================================================================
	-- 本SPのID
...
--         
        -- 銘柄_ステータス更新 (using PostgreSQL EXECUTE with dynamic SQL)
        gSql := 'UPDATE MGR_STS '
            || ' SET KK_STAT = $1'
            || ' WHERE SHORI_KBN = ''1'''
            || ' AND KK_PHASE = $2'
            || ' AND KK_STAT = $3'
            || ' AND ITAKU_KAISHA_CD = $4'
            || ' AND MGR_CD = $5';
        EXECUTE gSql USING l_inKkStat, gChkKkPhase, pkkknotice.MGR_KKSTAT_SHONIN(), gItakuKaishaCd, gMgrCd;
    ...
    ELSE
        ...
        -- Use dynamic SQL to avoid function calls in WHERE clause
        gSql := 'UPDATE MGR_STS'
            || ' SET MGR_SEND_TAISHO_FLG = ''1'','
            || '     SHONIN_KAIJO_YOKUSEI_FLG = ''0'''
            || ' WHERE ITAKU_KAISHA_CD = $1'
            || ' AND MGR_CD = $2'
            || ' AND SHORI_KBN = ''1'''
            || ' AND KK_PHASE = $3'
            || ' AND KK_STAT IN ($4, $5)';
        
        EXECUTE gSql USING gItakuKaishaCd, gMgrCd, gChkKkPhase, 
                          pkkknotice.MGR_KKSTAT_SHONIN(), pkkknotice.MGR_KKSTAT_SEND();
        GET DIAGNOSTICS sqlCount = ROW_COUNT;
    END IF;
    -- 更新件数チェック 
    IF sqlCount = 0 THEN
