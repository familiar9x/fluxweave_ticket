...
CREATE OR REPLACE FUNCTION sfipf009k00r09 () RETURNS integer AS $body$
DECLARE
...
-- * 会社名: JIP
-- *
-- * 「部店マスタ更新結果リスト」を呼び出し、戻り値により「バッチ帳票印刷管理」の登録を行う。
-- *
-- * @author 渡邊　かよ
...
BEGIN	
	...
	cGyoumuDt := pkDate.getGyomuYmd();
        ...
        FROM  SOWN_INFO;
	-- 部店マスタ更新結果リストの呼び出し
	   CALL SPIPF009K00R02(wk_kaiin, 'BATCH', '1', cGyoumuDt, rtn1, rtn2);
        -- 戻り値が'１'の場合
	IF  rtn1  =  1  THEN
            CALL pkLog.fatal(
           	     'ECM501',
        ...
        ELSIF  rtn1  IN (0)  THEN
            ...
            WHERE
                    ...
                    SC16.CHOHYO_ID = 'IPF30000931' AND
                    SC16.HEADER_FLG = 1;
            IF  ncount1  =  0  THEN
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION sfipf009k00r09 () FROM PUBLIC;
