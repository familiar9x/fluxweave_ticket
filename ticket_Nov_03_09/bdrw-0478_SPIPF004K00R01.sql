...
--					デバッグ機能
--==============================================================================
	DEBUG boolean := false;  -- Debug disabled
--==============================================================================
--					定数定義
--==============================================================================
	...
	REPORT_ID   CONSTANT char(11)  := 'IPF30000411';	-- 帳票ID
--==============================================================================
--					変数定義
--==============================================================================
...
--==============================================================================
	curMeisai CURSOR FOR
		...
		FROM mcalendar m61
		LEFT OUTER JOIN (
			SELECT * FROM SCODE WHERE CODE_SHUBETSU = '715'
		) sc04
...
BEGIN
	...
	IF COALESCE(TRIM(l_inItakuKaishaCd), '') = '' THEN
		...
		CALL pkLog.error(
			'ECM501',
			'IPF004K00R01',
			'＜項目名称:委託会社コード＞'||'＜項目値:'||l_inItakuKaishaCd||'＞'
		);
		RETURN;
	END IF;
	...
	IF COALESCE(TRIM(l_inChohyoSakuKbn), '') = '' THEN
		...
		CALL pkLog.error(
			'ECM501',
			'IPF004K00R01',
			'＜項目名称:帳票作成区分＞'||'＜項目値:'||l_inChohyoSakuKbn||'＞'
		);
		RETURN;
	END IF;
	...
	FOR recMeisai IN curMeisai LOOP
		IF DEBUG THEN
			RAISE NOTICE 'Loop: gCount=%, gAREA_CD=%, recMeisai.AREA_CD=%',
			             gCount, COALESCE(gAREA_CD, 'NULL'), recMeisai.AREA_CD;
		END IF;
		
		-- 14個に達したか、または地域コードが変わった場合
		IF gCount = 14
		   OR (gCount <> 0 AND gAREA_CD <> recMeisai.AREA_CD) THEN
			gSeqNo := gSeqNo + 1;
			
			IF DEBUG THEN
				RAISE NOTICE 'Inserting seq_no=%, area_cd=%, gCount=%',
				             gSeqNo, gAREA_CD, gCount;
			END IF;
