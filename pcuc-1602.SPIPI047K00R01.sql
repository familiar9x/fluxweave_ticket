


DROP TYPE IF EXISTS type_record;
CREATE TYPE type_record AS (
		gHktCd                 character(6)  -- 発行体コード
		,
		gSfskPostNo            character(7)  -- 送付先郵便番号
		,
		gAdd1                  character varying(50)  -- 送付先住所１
		,
		gAdd2                  character varying(50)  -- 送付先住所２
		,
		gAdd3                  character varying(50)  -- 送付先住所３
		,
		gHktNm                 character varying(100)  -- 発行体名称
		,
		gSfskBushoNm           character varying(50)  -- 送付先担当部署名称
		,
		gBankNm                character varying(100)  -- 銀行名称
		,
		gBushoNm1              character varying(40)  -- 担当部署名称１
		,
		gIsinCd                character(12)  -- ISINコード
		,
		gMgrNm                 character varying(200)  -- 銘柄の正式名称
		,
		gGanriKbn              varchar(10) -- 元利区分
		,
		gShrYmd                character(8)  -- 支払日
		,
		gShokanSeikyuKngk      numeric(16,2)  -- 元金支払額
		,
		gGankinTsukaNm         character varying(10)  -- 元金支払額通貨
		,
		gGzeihikiBefChokyuKngk numeric(16,2)  -- 利金支払額
		,
		gRikinTsukaNm          character varying(10)  -- 利金支払額通貨
		,
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--		gTaxNm                 character varying(60) -- 課税区分名称
--		,
--		gKokuZeiRate           numeric(15,13) -- 国税率
--		,
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
		gGzeiKngk              numeric(16,2)  -- 税金額
		,
		gZeikinTsukaNm         character varying(10)  -- 税金額通貨
		,
		gGzeihikiAftChokyuKngk numeric(16,2)  -- 利金支払基金請求額
		,
		gSeikyuTsukaNm         character varying(10)  -- 利金支払基金請求額通貨
		,
		gGnknShrTesuBunshi     numeric(10,7)  -- 元金手数料率分子
		,
		gGnknShrTesuBunbo      numeric(10,7)  -- 元金手数料率分母
		,
		gGnknShukinKngk        numeric(16,2)  -- 元金手数料
		,
		gRknShrTesuBunshi      numeric(10,7)  -- 利金手数料率分子
		,
		gRknShrTesuBunbo       numeric(10,7)  -- 利金手数料率分母
		,
		gRknShukinKngk         numeric(16,2)  -- 利金手数料
		,
		gTesuryoKomiFlg        character(1)  -- 手数料税込フラグ
		,
		gMgrCd                 character(14)  -- 銘柄コード
		,
		gTaxKbn				   character(2)  -- 課税区分
		,
		gEigyotenCd			   character(4)  -- 営業店コード
		,
		gEigyotenNm			   character varying(50)  -- 営業店名称
		,
		gRikinHasu			   numeric(16,2)  -- 利金端数
	);


CREATE OR REPLACE PROCEDURE spipi047k00r01 ( l_inItakuKaishaCd TEXT, -- 委託会社コード
 l_inUserId TEXT, -- ユーザーID
 l_inChohyoKbn TEXT, -- 帳票区分
 l_inGyomuYmd TEXT, -- 業務日付　←夜間はここまで
 l_inHktCd TEXT, -- 発行体コード
 l_inKozaTenCd TEXT, -- 口座店コード
 l_inKozaTenCifCd TEXT, -- 口座店CIFコード
 l_inMgrCd TEXT, -- 銘柄コード
 l_inIsinCd TEXT, -- ISINコード
 l_inKijunYm TEXT, -- 基準年月
 l_inKijunYmdFrom TEXT, -- 基準日From
 l_inKijunYmdTo TEXT, -- 基準日To
 l_inTsuchiYmd TEXT, -- 通知日
 l_outSqlCode OUT integer, -- リターン値
 l_outSqlErrM OUT text -- エラーコメント
 ) AS $body$
DECLARE

	--
--	 * 著作権:Copyright(c)2004
--	 * 会社名:JIP
--	 * 概要　:画面から、元利金支払報告書を作成する。
--	 *     
--	 * ■「グロスアップ銘柄税率入力OP」の追加処理概要
--	 *    前提として、本帳票は基準年月指定の発行体・ＩＳＩＮ毎でブレイク処理される帳票である。
--	 *
--	 *    ①作表SQLの実行時の取得明細レコードcurMeisaiのカーソルループ中の処理により、
--	 *    各明細レコード.課税区分がグロスアップ銘柄対象判定(10, 20, 90, 92, 94 のいずれか？)をおこなう。【isGrossupTaxKbn】
--	 *    
--	 *    ②対象課税区分であれば、明細レコード.支払日の支払期日を銘柄利払回次情報から抽出をおこない、
--	 *    該当支払期日のグロスアップ銘柄情報を取得する。【getGrossupKokuZeiRitsu】
--	 *    
--	 *    ③②の処理によりグロスアップ銘柄の適用利払期日が抽出できれば、
--	 *    明細レコード.支払日以降の税率欄を「（※連番）」表記の文字列に置きかえる。
--	 *    グロスアップ銘柄税率の適用利払期日毎（【getGrossupKokuZeiRitsu】から取得した適用利払期日が
--	 *    前ループにおいて取得した適用利払期日と異なるたび）に　（※連番）国税率　表記を追記した欄外国税率文字列（注１）を生成する。　
--	 *    尚、国税率は帳票最下部欄外に最大５つまで表記する。
--	 *    (作成文字列　注１の例)
--	 *       ※１　7.2653785437684　※２　8.7130489805724　※３　6.6668837126266　※４　10.2985483385089　※５　9.2090257281829
--	 *    
--	 *    ④ブレイク毎にすべて同一の国税率を欄外表記をさせるため、ブレイクが発生するタイミングにより、
--	 *    ページ毎の最終明細レコードの国税率欄外情報フィールド（ITEM41）に欄外国税率文字列（注１）の更新をかける。
--	 *    （SREPORT_WKの１レコードのみのITEM41の更新であっても、その頁の欄外の国税率表示には反映される。）
--	 *
--	 *    この時、最終ブレイク情報の欄外国税率文字列の更新タイミングがループ中に無いため、
--	 *    ループ終了後に更新処理をする。【writeRangaiKokuZeiRateList】
--	 *    
--	 *    ⑤ブレイク発生毎にグロスアップ銘柄税率表記情報を初期化する。
--	 *    
--	 *    
--	 * 引数　:l_inItakuKaishaCd    IN  TEXT      委託会社コード
--	 *       l_inUserId           IN  TEXT      ユーザーID
--	 * 　　　 l_inChohyoKbn        IN  TEXT      帳票区分
--	 * 　　　 l_inGyomuYmd         IN  TEXT      業務日付
--	 * 　　　 l_inHktCd            IN  TEXT      発行体コード
--	 * 　　　 l_inKozaTenCd        IN  TEXT      口座店コード
--	 * 　　　 l_inKozaTenCifCd     IN  TEXT      口座店CIFコード
--	 * 　　　 l_inMgrCd            IN  TEXT      銘柄コード
--	 * 　　　 l_inIsin             IN  TEXT      ISINコード
--	 * 　　　 l_inKijunYm          IN  TEXT      基準年月
--	 *       l_inKijunYmdFrom	  IN  TEXT		基準日From
--	 *		 l_inKijunYmdTo		  IN  TEXT		基準日To
--	 * 　　　 l_inTsuchiYmd        IN  TEXT      通知日
--	 * 　　　 l_outSqlCode         OUT INTEGER   リターン値
--	 * 　　　 l_outSqlErrM         OUT VARCHAR  エラーコメント
--	 * 返り値:なし
--	 * @version $Id: SPIPI047K00R01.sql,v 1.33.2.2 2025/07/30 01:50:52 taira Exp $ 
--     
	--==============================================================================
	--                デバッグ機能                                                   
	--==============================================================================
	--==============================================================================
	--                定数定義                                                      
	--==============================================================================
	RTN_OK     CONSTANT integer := 0; -- 正常
	RTN_NG     CONSTANT integer := 1; -- 予期したエラー
	RTN_NODATA CONSTANT integer := 2; -- データなし
	RTN_FATAL  CONSTANT integer := 99; -- 予期せぬエラー
	REPORT_ID      CONSTANT text := 'IP030004711'; -- 元利金支払報告書帳票ID
	TSUCHI_YMD_DEF CONSTANT text := '      年  月  日'; -- 平成10年10月10日
	--==============================================================================
	--                変数定義                                                      
	--==============================================================================
	v_item type_sreport_wk_item;						-- Composite type for pkPrint.insertData
	gRtnCd integer := RTN_OK; -- リターンコード
	gSeqNo integer := 0; -- シーケンス
	gNo    integer := 0; -- NO
	gSQL   varchar(10000) := NULL; -- SQL編集
	-- 書式フォーマット
	gGyomuYm           char(6) := NULL; -- 業務年月(リアルと夜間で変わる)
	gKijunYm		   char(6) := NULL; -- (IP-05244)
	gWrkTsuchiYmd      varchar(16) := NULL; -- 通知日(西暦)
	gAtena             varchar(200) := NULL; -- 宛名
	gOutflg            numeric := 0; -- 正常処理フラグ
	gWrkToriYm         varchar(14) := NULL; -- 取扱年月
	gBunsho            varchar(200) := NULL; -- 請求文章
	gGnknShrGoukei     decimal(16,2) := 0; -- 元金支払額合計
	gRknShrGoukei      decimal(16,2) := 0; -- 利金支払額合計
	gZeiGoukei         decimal(16,2) := 0; -- 税金額合計
	gKknSeiGoukei      decimal(16,2) := 0; -- 利金支払基金請求額合計
	gTesuGoukei        decimal(16,2) := 0; -- 手数料合計
	gGnknShrTesuBunshi decimal(10,7) := 0; -- 元金支払手数料率_分子
	gRknShrTesuBunshi  decimal(10,7) := 0; -- 利金支払手数料率_分子
	gRknTesuTsuka      char(3)		 := '円 ';	-- 利金手数料通貨コード
	gKokuZeiRateFm		varchar(21) := NULL;					-- 国税率
	gGZeiKngkFm			varchar(21) := NULL;					-- 国税額
	gGzeihikiAftChokyuKngkFm	varchar(21) := NULL;					-- 税引後利金額
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
	--税区分取得用変数
	gRbrKjt            character(8);
	gRet			   numeric := 0;
	gTaxNm             character varying(60) := NULL;			--税区分名称
	gTaxRnm            character varying(6) := NULL;		--税区分略称
	gKokuZeiRate       numeric(15,13) := NULL;	--国税率
	gChihoZeiRate      numeric(5,3) := NULL;	--地方税率
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	--  グロスアップ用ワーク変数
	GRS_OPTION_CD  				CONSTANT character(13) := 'IPX1011112010'; 		--オプションID
	GRS_FMT 					CONSTANT text := 'FM99999999990.0000000000000'; 	--グロスアップ国税率用フォーマット
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--	TAX_FMT						CONSTANT CHAR(7)  := 'FM90.00';				--通常税率フォーマット
	TAX_FMT						CONSTANT text := 'FM90.00000';				--通常税率フォーマット
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	ROW_MAX						CONSTANT integer := 8; -- 元利金支払報告書の最大詳細行
	GRS_CNT_MAX					CONSTANT integer := 5; -- グロスアップ銘柄税率設定最大値
	grsRangaiChuki              varchar(200) := NULL;	-- 欄外注記の文言
	grsKokuZeiRate				decimal(15,13)  := 0;					-- 国税率			
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--	grsVKokuZeiRate   			VARCHAR(5) DEFAULT NULL; 					-- 表示用国税率
	grsVKokuZeiRate   			varchar(10) := NULL; 					-- 表示用国税率
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	grsStrKokuZeiRate   		varchar(16) := NULL; 					-- 文字列国税率
	grsRangaiKokuZeiRateList	varchar(164) := NULL;					-- 欄外表示の国税率リスト		
	grsVRangaiKokuZeiRateList	varchar(164) := NULL;					-- 欄外表示の国税率リスト		
	grsGrossSeqNo 				integer := 0;							-- シーケンス			
	grsAst 						char(4) := NULL;						-- ※			
	grsShrYmd					character(8) := NULL;					-- 適用利払期日
	grsTekiyoRbr				character(8) := NULL;					-- 適用利払期日
	grsPrTekiyoRbr				character(8) := NULL;					-- 前適用利払期日
	-- IP-05983 営業店コード・名称表示対応変数
	gEigyoutenCdNm				varchar(66) := NULL;					-- 営業店コード・名称
	
	
	
	-- ブレイクキー
	keyHktCd  character(6) := ''; -- 発行体コード
	keyIsinCd character(12) := ''; -- ISINコード
	-- DB取得項目
	-- 配列定義
	recMeisai TYPE_RECORD; -- レコード
	gChohyoSortFlg		character varying(200);				-- 発行体宛帳票ソート順変更フラグ
	gEigyoutenHyoujiFlg	character varying(200);				-- 営業店コード・名称表示フラグ
	gRikinHasuFlg		character varying(200);				-- その他基金出金 利金端数計算フラグ取得
	gTesuryoHasuFlg		character varying(200);				-- その他基金出金 手数料端数計算フラグ取得
	gShzKijunProcess	character varying(200);				-- 消費税率適用基準日対応
	gShzKijunYmd		varchar(8);								-- 消費税率適用基準日
	gInvoiceFlg		character(1);				-- オプションフラグ取得	
	gGnrshr0outFlg		character varying(200);				-- 元利金支払報告書作成制御フラグ
	gChohyoHankoFileNm	varchar(400);								-- 帳票印影の画像ファイルの絶対パス
	
	-- カーソル
	
	curMeisai REFCURSOR;
--==============================================================================
--                メイン処理                                                       
--==============================================================================
BEGIN
	-- 入力パラメータのチェック
	IF coalesce(trim(both l_inItakuKaishaCd)::text, '') = '' OR coalesce(trim(both l_inUserId)::text, '') = '' OR
		coalesce(trim(both l_inChohyoKbn)::text, '') = '' OR coalesce(trim(both l_inGyomuYmd)::text, '') = '' THEN
			-- パラメータエラー
			l_outSqlCode := RTN_NG;
			l_outSqlErrM := '';
			CALL pkLog.error('ECM501', REPORT_ID, 'SQLERRM:' || '');
			RETURN;
    ELSIF l_inChohyoKbn = '0' THEN
          IF (coalesce(trim(both l_inKijunYm)::text, '') = '') AND (coalesce(trim(both l_inKijunYmdFrom)::text, '') = '' OR coalesce(trim(both l_inKijunYmdTo)::text, '') = '') THEN
                -- パラメータエラー
			l_outSqlCode := RTN_NG;
			l_outSqlErrM := '';
			CALL pkLog.error('ECM501', REPORT_ID, 'SQLERRM:' || '');
			RETURN;
          END IF;
	END IF;
	-- 帳票区分による初期設定
	IF l_inChohyoKbn = '0' THEN
		-- リアル
		-- 業務年月
		gGyomuYm := l_inKijunYm;
		-- 通知日(西暦)
		IF (trim(both l_inTsuchiYmd) IS NOT NULL AND (trim(both l_inTsuchiYmd))::text <> '') THEN
			gWrkTsuchiYmd := pkDate.seirekiChangeSuppressNenGappi(l_inTsuchiYmd);
		ELSE
			gWrkTsuchiYmd := TSUCHI_YMD_DEF;
		END IF;
		-- ⑥基準日の年月、⑦基準年月の年月どちらかが来るのでセット
		gKijunYm := trim(both l_inKijunYm);
		IF (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
			gKijunYm := substr(l_inKijunYmdFrom, 1, 6);
		END IF;
		-- 取扱年月取得
		gWrkToriYm := substr(pkDate.seirekiChangeSuppressNenGappi(gKijunYm || '01'),
							  1,
							  10);
		gWrkToriYm := gWrkToriYm || '分';
	ELSE
		-- 夜間
		-- 業務年月
		gGyomuYm := substr(l_inGyomuYmd, 1, 6);
		-- 通知日(翌営業日西暦)
		gWrkTsuchiYmd := pkDate.getYokuBusinessYmd(l_inGyomuYmd);
		gWrkTsuchiYmd := pkDate.seirekiChangeSuppressNenGappi(gWrkTsuchiYmd);
		-- 取扱年月取得
		gWrkToriYm := substr(pkDate.seirekiChangeSuppressNenGappi(l_inGyomuYmd),
							  1,
							  10);
		gWrkToriYm := gWrkToriYm || '分';
	END IF;
	-- 請求文章取得
	gBunsho := SPIPI047K00R01_createBun(REPORT_ID, '00');
	-- 帳票ワークの削除
	DELETE FROM SREPORT_WK
		WHERE KEY_CD = l_inItakuKaishaCd
		AND USER_ID = l_inUserId
		AND CHOHYO_KBN = l_inChohyoKbn
		AND SAKUSEI_YMD = l_inGyomuYmd
		AND CHOHYO_ID = REPORT_ID;
	--発行体宛帳票ソート順変更フラグ取得
	gChohyoSortFlg := pkControl.getCtlValue(l_inItakuKaishaCd, 'SeikyusyoSort', '0');
	--営業店コード・名称表示フラグ取得
	gEigyoutenHyoujiFlg := pkControl.getCtlValue(l_inItakuKaishaCd, 'SPIPI047K00R01', '0');
	--その他基金出金 利金端数計算フラグ取得
	gRikinHasuFlg := pkControl.getCtlValue(l_inItakuKaishaCd, 'SPIPI047K00R012', '0');
	--その他基金出金 手数料端数計算フラグ取得
	gTesuryoHasuFlg := pkControl.getCtlValue(l_inItakuKaishaCd, 'SPIPI047K00R013', '0');
	-- 消費税率適用基準日フラグ(1:取引日ベース, 0:徴求日ベース(デフォルト))取得
	gShzKijunProcess := pkControl.getCtlValue(l_inItakuKaishaCd, 'ShzKijun', '0');
	-- 元利金支払報告書作成制御フラグ取得
	gGnrshr0outFlg := pkControl.getCtlValue(l_inItakuKaishaCd, 'Gnrshr0out', '0');
	-- オプションフラグ取得
	gInvoiceFlg := pkControl.getOPTION_FLG(l_inItakuKaishaCd, 'INVOICE_A', '0');
	--みずほリリース対象外対応
	gInvoiceFlg := '0';
	-- SQL編集
	CALL SPIPI047K00R01_createSQL(
		gRikinHasuFlg,
		gTesuryoHasuFlg,
		l_inChohyoKbn,
		l_inKijunYmdFrom,
		l_inKijunYmdTo,
		gGyomuYm,
		gGnrshr0outFlg,
		l_inItakuKaishaCd,
		l_inHktCd,
		l_inKozaTenCd,
		l_inKozaTenCifCd,
		l_inMgrCd,
		l_inIsinCd,
		gChohyoSortFlg,
		gSQL
	);
	-- 帳票印影の画像ファイルの絶対パスを取得する
	gChohyoHankoFileNm := pkIpaChohyoHanko.getChohyoHankoFileName(l_inItakuKaishaCd);
	-- ヘッダレコードを追加
	CALL pkPrint.insertHeader(l_inItakuKaishaCd,
						l_inUserId,
						l_inChohyoKbn,
						l_inGyomuYmd,
						REPORT_ID);
	RAISE NOTICE '[TRACE] Header inserted, about to open cursor';
	RAISE NOTICE '[TRACE] Generated SQL: %', gSQL;
	-- カーソルオープン
	OPEN curMeisai FOR EXECUTE gSQL;
	RAISE NOTICE '[TRACE] Cursor opened successfully';
	-- データ取得
	LOOP
		FETCH curMeisai
			INTO recMeisai.gHktCd,				-- 発行体コード
			recMeisai.gSfskPostNo,				-- 送付先郵便番号
			recMeisai.gAdd1,					-- 送付先住所１
			recMeisai.gAdd2,					-- 送付先住所２
			recMeisai.gAdd3,					-- 送付先住所３
			recMeisai.gHktNm,					-- 発行体名称
			recMeisai.gSfskBushoNm,				-- 送付先担当部署名称
			recMeisai.gBankNm,					-- 銀行名称
			recMeisai.gBushoNm1,				-- 担当部署名称１
			recMeisai.gIsinCd,					-- ISINコード
			recMeisai.gMgrNm,					-- 銘柄の正式名称
			recMeisai.gGanriKbn,				-- 元利区分
			recMeisai.gShrYmd,					-- 支払日
			recMeisai.gShokanSeikyuKngk,		-- 元金支払額
			recMeisai.gGankinTsukaNm,			-- 元金支払額通貨
			recMeisai.gGzeihikiBefChokyuKngk,	-- 利金支払額
			recMeisai.gRikinTsukaNm,			-- 利金支払額通貨
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--			recMeisai.gTaxNm,					-- 課税区分名称
--			recMeisai.gKokuZeiRate,				-- 国税率
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
			recMeisai.gGzeiKngk,				-- 税金額
			recMeisai.gZeikinTsukaNm,			-- 税金額通貨
			recMeisai.gGzeihikiAftChokyuKngk,	-- 利金支払基金請求額
			recMeisai.gSeikyuTsukaNm,			-- 利金支払基金請求額通貨
			recMeisai.gGnknShrTesuBunshi,		-- 元金手数料率分子
			recMeisai.gGnknShrTesuBunbo,		-- 元金手数料率分母
			recMeisai.gGnknShukinKngk,			-- 元金手数料
			recMeisai.gRknShrTesuBunshi,		-- 利金手数料率分子
			recMeisai.gRknShrTesuBunbo,			-- 利金手数料率分母
			recMeisai.gRknShukinKngk,			-- 利金手数料
			recMeisai.gTesuryoKomiFlg,			-- 手数料税込フラグ
			recMeisai.gMgrCd,					-- 銘柄コード
			recMeisai.gTaxKbn,					-- 課税区分
			recMeisai.gEigyotenCd,				-- 営業店コード
			recMeisai.gEigyotenNm,				-- 営業店名称
			recMeisai.gRikinHasu 				-- 利金端数
			;
	-- データが無くなったらループを抜ける
	EXIT WHEN NOT FOUND;/* apply on curMeisai */
	-- 宛名編集
	CALL pkIpaName.getMadoFutoAtenaYoko(	recMeisai.gHktNm,
									recMeisai.gSfskBushoNm,
									gOutflg,
									gAtena);
	-- 手数料率分子の取得(元金支払手数料率_分子、利金支払手数料率_分子)
	gGnknShrTesuBunshi := 0;
	IF (recMeisai.gGnknShrTesuBunshi IS NOT NULL AND recMeisai.gGnknShrTesuBunshi::text <> '') THEN
		-- 消費税率適用基準日切り替え
		IF gShzKijunProcess = '1' THEN
			gShzKijunYmd := recMeisai.gShrYmd;
		ELSE
			-- 基金異動履歴から元金支払手数料徴求日を取得する
			gShzKijunYmd := PKIPACALCTESURYO.getTesuChokyuYmd(l_inItakuKaishaCd, recMeisai.gMgrCd, recMeisai.gShrYmd, '12');
		END IF;
		gGnknShrTesuBunshi := pkTesuryoRitsu.getTesuryoRitsuBunshi(	l_inItakuKaishaCd,
																	recMeisai.gGnknShrTesuBunshi,
																	gShzKijunYmd,
																	recMeisai.gMgrCd,
																	gOutflg);
	END IF;
	gRknShrTesuBunshi := 0;
	IF (recMeisai.gRknShrTesuBunshi IS NOT NULL AND recMeisai.gRknShrTesuBunshi::text <> '') THEN
		-- 消費税率適用基準日切り替え
		IF gShzKijunProcess = '1' THEN
			gShzKijunYmd := recMeisai.gShrYmd;
		ELSE
			-- 基金異動履歴から利金支払手数料徴求日を取得する
			gShzKijunYmd := PKIPACALCTESURYO.getTesuChokyuYmd(l_inItakuKaishaCd, recMeisai.gMgrCd, recMeisai.gShrYmd, '22');
		END IF;
		gRknShrTesuBunshi := pkTesuryoRitsu.getTesuryoRitsuBunshi(	l_inItakuKaishaCd,
																	recMeisai.gRknShrTesuBunshi,
																	gShzKijunYmd,
																	recMeisai.gMgrCd,
																	gOutflg);
	END IF;
	-- ブレイク処理(発行体コード、ISINコード)
	IF keyHktCd <> recMeisai.gHktCd OR keyIsinCd <> recMeisai.gIsinCd THEN
		--グロスアップ銘柄税率入力ＯＰフラグの有無
		IF pkControl.getOPTION_FLG(l_inItakuKaishaCd , GRS_OPTION_CD,'0') = '1' THEN
			--グロスアップ銘柄税率設定　ブレイク処理時処理
			--欄外表示の国税率リストの設定
			IF gSeqNo > 0 THEN
				--「グロスアップ銘柄税率入力OP」の追加処理概要④
				CALL SPIPI047K00R01_writeRangaiKokuZeiRateList(
				  	l_inItakuKaishaCd	     	,
					l_inUserId     	,
					l_inChohyoKbn  	,
					l_inGyomuYmd 	,
					REPORT_ID		,
					gSeqNo    		,
					gNo   				,
					grsRangaiKokuZeiRateList,
					grsRangaiChuki
				);
			END IF;
			--「グロスアップ銘柄税率入力OP」の追加処理概要⑤
			--ブレイク処理時のリセット　初期化
			grsGrossSeqNo := 0;
			grsRangaiKokuZeiRateList := '';
			grsPrTekiyoRbr := '';
			grsTekiyoRbr := '';
			grsRangaiChuki := '';
		END IF;
		-- 合計変数初期化
		gGnknShrGoukei := 0;
		gRknShrGoukei  := 0;
		gZeiGoukei     := 0;
		gKknSeiGoukei  := 0;
		gTesuGoukei    := 0;
		-- NO初期化
		gNo := 0;
	END IF;
	-- 合計算出
	gGnknShrGoukei := gGnknShrGoukei + coalesce(recMeisai.gShokanSeikyuKngk, 0);
	gRknShrGoukei  := gRknShrGoukei +
						coalesce(recMeisai.gGzeihikiBefChokyuKngk, 0);
	gZeiGoukei     := gZeiGoukei + coalesce(recMeisai.gGzeiKngk, 0);
	gKknSeiGoukei  := gKknSeiGoukei +
						coalesce(recMeisai.gGzeihikiAftChokyuKngk, 0);
	-- ブレイクキー設定
	keyHktCd  := recMeisai.gHktCd;
	keyIsinCd := recMeisai.gIsinCd;
	-- シーケンスアップ
	gSeqNo := gSeqNo + 1;
	-- NOアップ
	gNo := gNo + 1;
	-- 手数料率の分子がゼロの場合はブランクにする
	IF gGnknShrTesuBunshi <= 0 THEN
		gGnknShrTesuBunshi := NULL;
		recMeisai.gGnknShrTesuBunbo := NULL;
	END IF;
	IF gRknShrTesuBunshi <= 0 THEN
		gRknShrTesuBunshi := NULL;
		recMeisai.gRknShrTesuBunbo := NULL;
	END IF;
	-- 支払額の合計がゼロの場合は手数料をブランクにする
	-- 支払額の合計がゼロ以外で、手数料率がブランクの場合は、手数料をゼロにする
	IF gGnknShrGoukei <= 0 THEN
		recMeisai.gGnknShukinKngk := NULL;
	ELSE
		IF coalesce(gGnknShrTesuBunshi::text, '') = '' THEN
			recMeisai.gGnknShukinKngk := 0;
		END IF;
	END IF;
	IF (gRknShrGoukei <= 0 AND recMeisai.gRknShukinKngk = 0) THEN
		recMeisai.gRknShukinKngk := NULL;
	ELSE
		IF coalesce(gRknShrTesuBunshi::text, '') = '' THEN
			recMeisai.gRknShukinKngk := 0;
		END IF;
	END IF;
	-- 手数料種類に応じて利払手数料通貨コードを取得(元金ベース=発行通貨コード、利金ベース=利払通貨コード)
	BEGIN
		SELECT (SELECT TSUKA_NM FROM MTSUKA WHERE TSUKA_CD = CASE WHEN MG7.TESU_SHURUI_CD='61' THEN MG1.HAKKO_TSUKA_CD WHEN MG7.TESU_SHURUI_CD='82' THEN MG1.RBR_TSUKA_CD END )
		INTO STRICT
			gRknTesuTsuka
		FROM
			MGR_KIHON MG1,
			(SELECT
				MG7.ITAKU_KAISHA_CD,
				MG7.MGR_CD,
				MG7.TESU_SHURUI_CD
			FROM	MGR_TESURYO_CTL MG7
				WHERE	MG7.TESU_SHURUI_CD IN ('61', '82')
					AND	MG7.CHOOSE_FLG = '1' ) MG7
		WHERE	MG1.ITAKU_KAISHA_CD		= MG7.ITAKU_KAISHA_CD
			AND	MG1.MGR_CD				= MG7.MGR_CD
			AND	MG1.ITAKU_KAISHA_CD		= l_inItakuKaishaCd
			AND	MG1.MGR_CD				= recMeisai.gMgrCd;
	EXCEPTION
		WHEN no_data_found THEN
			gRknTesuTsuka := '';
	END;
	-- 手数料合計を算出する
	IF recMeisai.gGankinTsukaNm = gRknTesuTsuka OR
		 coalesce(recMeisai.gGnknShukinKngk, 0) = 0 OR coalesce(recMeisai.gRknShukinKngk, 0) = 0 THEN
		gTesuGoukei := coalesce(recMeisai.gGnknShukinKngk, 0) + coalesce(recMeisai.gRknShukinKngk, 0);
	END IF;
	IF coalesce(recMeisai.gGankinTsukaNm::text, '') = '' AND (gRknTesuTsuka IS NOT NULL AND gRknTesuTsuka::text <> '') THEN
		gTesuGoukei := recMeisai.gRknShukinKngk;
	END IF;
	IF (recMeisai.gGankinTsukaNm IS NOT NULL AND recMeisai.gGankinTsukaNm::text <> '') AND coalesce(gRknTesuTsuka::text, '') = '' THEN
		gTesuGoukei := recMeisai.gGnknShukinKngk;
	END IF;
	-- 元利金支払手数料、利金支払手数料がブランクなら手数料合計もブランクにする
	IF coalesce(recMeisai.gGnknShukinKngk::text, '') = '' AND coalesce(recMeisai.gRknShukinKngk::text, '') = '' THEN
		gTesuGoukei := NULL;
	END IF;
	-- 元利金支払手数料、利金支払手数料の通貨が異なり、かつ、その両方に金額がある場合もブランクにする
	IF recMeisai.gGankinTsukaNm <> gRknTesuTsuka AND
		 coalesce(recMeisai.gGnknShukinKngk, 0) > 0 AND coalesce(recMeisai.gRknShukinKngk, 0) > 0 THEN
		gTesuGoukei := NULL;
	END IF;
	-- 手数料合計が0円の場合もブランクにする
	IF gTesuGoukei = 0 THEN
		gTesuGoukei := NULL;
	END IF;
	-- 元金手数料のみの場合、利金手数料通貨にも元金とおなじ通貨をセット(手数料合計に表示するため)
	IF recMeisai.gRknShukinKngk = 0 OR coalesce(trim(both recMeisai.gRknShukinKngk)::text, '') = '' THEN
		gRknTesuTsuka := recMeisai.gGankinTsukaNm;
	END IF;
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
	--元利払期日取得
	gRbrKjt := pkIpaZei.getIpaRbrYmd(l_inItakuKaishaCd,
									 recMeisai.gMgrCd,
									 recMeisai.gShrYmd,
									 recMeisai.gTaxKbn);
	--元利払期日に該当する適用開始日の税区分を取得する
	gRet := pkIpaZei.getMTax(recMeisai.gTaxKbn,
							 gRbrKjt,
							 gTaxNm,
							 gTaxRnm,
							 gKokuZeiRate,
							 gChihoZeiRate
							);
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	
	--グロスアップ銘柄税率入力ＯＰを使用しないくとも、以下の変数名で処理する。
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--	grsKokuZeiRate		:= recMeisai.gKokuZeiRate;	
	grsKokuZeiRate		:= gKokuZeiRate;	
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	grsVKokuZeiRate		:= oracle.to_char(grsKokuZeiRate,TAX_FMT);	
	--グロスアップ銘柄税率入力ＯＰフラグの有無
	IF pkControl.getOPTION_FLG(l_inItakuKaishaCd , GRS_OPTION_CD,'0') = '1' THEN
		IF recMeisai.gTaxKbn = '90' THEN   --課税区分が'90'の場合
		 --国税率欄の※を設定する
			grsVKokuZeiRate := '※';
			IF coalesce(trim(both grsRangaiChuki)::text, '') = '' THEN  --(欄外表示の注記が未設定)　最初のループであれば設定しておく
				grsRangaiChuki :='※同じ課税区分ながら異なる税率が適用されるため、税率を非表示としております。';
			END IF;
		END IF;
			--「グロスアップ銘柄税率入力OP」の追加処理概要 ②	
			CALL SPIPI047K00R01_getGrossupKokuZeiRitsu(l_inItakuKaishaCd,recMeisai.gMgrCd,recMeisai.gShrYmd,recMeisai.gTaxKbn,grsKokuZeiRate,grsTekiyoRbr);
			--抽出できれば、「grsKokuZeiRate」に設定される。
			IF (grsKokuZeiRate IS NOT NULL AND grsKokuZeiRate::text <> '') THEN									
				IF coalesce(trim(both grsRangaiKokuZeiRateList)::text, '') = '' THEN								
					grsRangaiKokuZeiRateList := 'グロスアップ銘柄の税率は次のとおりです。';						
				END IF;								
				--「グロスアップ銘柄税率入力OP」の追加処理概要 ③	
				IF  coalesce(trim(both grsPrTekiyoRbr)::text, '') = '' OR
					--前回と今回で適用利払日付に差がある
					( trim(both grsTekiyoRbr) <> trim(both grsPrTekiyoRbr) ) AND
					--グロスアップ銘柄件数の最大カウント以下
					grsGrossSeqNo < GRS_CNT_MAX
					THEN
					grsPrTekiyoRbr := grsTekiyoRbr;				
					--帳票表出力文字列として、フォーマットする
					grsStrKokuZeiRate := oracle.to_char(grsKokuZeiRate,GRS_FMT);	
					--グロスアップ適用利払数をカウントアップ
					grsGrossSeqNo := grsGrossSeqNo + 1;					
					--文字列生成　（※連番）　
					grsAst := '※' || TRANSLATE(grsGrossSeqNo::text,'12345','１２３４５');								
					grsVKokuZeiRate :=grsAst;	
					--国税率欄外リスト文字列を連結処理をする	
					grsRangaiKokuZeiRateList := grsRangaiKokuZeiRateList || '　' || grsAst || '　' || grsStrKokuZeiRate;							
				ELSE
				    --グロスアップ対象銘柄であるが適用利払日が前回と同様であるため、そのまま国税率表記を利用する。
					grsVKokuZeiRate :=grsAst;
				END IF;										
			END IF;			
	END IF;
	-- IP-05983 営業店コード・名称表示対応
	-- 営業店コード・名称表示フラグが1の時、営業店コード・名称を入れる
	IF gEigyoutenHyoujiFlg = '1' THEN
   		gEigyoutenCdNm      := '営業店（ ' || recMeisai.gEigyotenCd || ' ' ||
												recMeisai.gEigyotenNm || '）';  -- 営業店コード・名称
    ELSE
    	gEigyoutenCdNm      := '';
    END IF;
    -- 税区分が'85'の場合、以下をセットする
		IF recMeisai.gTaxKbn = '85' THEN
		    gKokuZeiRateFm := '-';			-- 国税率
		    gGZeiKngkFm := '-';				-- 国税額
		    gGzeihikiAftChokyuKngkFm := '-';			-- 利金支払基金請求額
		ELSE
			gKokuZeiRateFm := grsVKokuZeiRate;			-- 国税率
		    gGZeiKngkFm := recMeisai.gGzeiKngk;			-- 国税額
		    gGzeihikiAftChokyuKngkFm := recMeisai.gGzeihikiAftChokyuKngk;			-- 利金支払基金請求額
		END IF;
	-- 帳票ワークへデータを追加
			-- Clear composite type
		v_item := ROW();
		
		v_item.l_inItem001 := gWrkTsuchiYmd;	-- 通知日
		v_item.l_inItem002 := recMeisai.gHktCd;	-- 発行体コード
		v_item.l_inItem003 := recMeisai.gSfskPostNo;	-- 送付先郵便番号
		v_item.l_inItem004 := recMeisai.gAdd1;	-- 送付先住所１
		v_item.l_inItem005 := recMeisai.gAdd2;	-- 送付先住所２
		v_item.l_inItem006 := recMeisai.gAdd3;	-- 送付先住所３
		v_item.l_inItem007 := gAtena;	-- 発行体名称１(御中込)
		v_item.l_inItem008 := recMeisai.gBankNm;	-- 銀行名称
		v_item.l_inItem009 := recMeisai.gBushoNm1;	-- 担当部署名称１
		v_item.l_inItem010 := gBunsho;	-- 請求文章
		v_item.l_inItem011 := gWrkToriYm;	-- 取扱年月
		v_item.l_inItem012 := recMeisai.gIsinCd;	-- ISINコード
		v_item.l_inItem013 := recMeisai.gMgrNm;	-- 銘柄の正式名称
		v_item.l_inItem014 := gNo;	-- NO
		v_item.l_inItem015 := recMeisai.gGanriKbn;	-- 元利区分
		v_item.l_inItem016 := recMeisai.gShrYmd;	-- 支払日
		v_item.l_inItem017 := recMeisai.gShokanSeikyuKngk;	-- 元金支払額
		v_item.l_inItem018 := recMeisai.gGankinTsukaNm;	-- 元金支払額通貨
		v_item.l_inItem019 := recMeisai.gGzeihikiBefChokyuKngk;	-- 利金支払額
		v_item.l_inItem020 := recMeisai.gRikinTsukaNm;	-- 利金支払額通貨
		v_item.l_inItem021 := gTaxNm;	-- 課税区分
		v_item.l_inItem022 := gKokuZeiRateFm;	-- 国税率
		v_item.l_inItem023 := gGZeiKngkFm;	-- 税金額
		v_item.l_inItem024 := recMeisai.gZeikinTsukaNm;	-- 税金額通貨
		v_item.l_inItem025 := gGzeihikiAftChokyuKngkFm;	-- 利金支払基金請求額
		v_item.l_inItem026 := recMeisai.gSeikyuTsukaNm;	-- 利金支払基金請求額通貨
		v_item.l_inItem027 := gGnknShrTesuBunshi;	-- 元金手数料率分子
		v_item.l_inItem028 := recMeisai.gGnknShrTesuBunbo;	-- 元金手数料率分母
		v_item.l_inItem029 := recMeisai.gGnknShukinKngk;	-- 元金手数料
		v_item.l_inItem030 := gRknShrTesuBunshi;	-- 利金手数料率分子
		v_item.l_inItem031 := recMeisai.gRknShrTesuBunbo;	-- 利金手数料率分母
		v_item.l_inItem032 := recMeisai.gRknShukinKngk;	-- 利金手数料
		v_item.l_inItem033 := recMeisai.gTesuryoKomiFlg;	-- 手数料税込フラグ
		v_item.l_inItem034 := gGnknShrGoukei;	-- 元金支払額合計
		v_item.l_inItem035 := gRknShrGoukei + coalesce(recMeisai.gRikinHasu, 0);	-- 利金支払額合計
		v_item.l_inItem036 := gZeiGoukei;	-- 税金額合計
		v_item.l_inItem037 := gKknSeiGoukei + coalesce(recMeisai.gRikinHasu, 0);	-- 利金支払基金請求額合計
		v_item.l_inItem038 := gTesuGoukei;	-- 手数料合計
		v_item.l_inItem040 := gRknTesuTsuka;	-- 利金手数料通貨
		v_item.l_inItem041 := '';	-- 新規変更
		v_item.l_inItem042 := '';	-- 新規変更
		v_item.l_inItem043 := gEigyoutenCdNm;	-- IP-05983 営業店コード・名称表示対応
		v_item.l_inItem044 := gRikinHasuFlg;	-- 利金端数計算フラグ
		v_item.l_inItem045 := gTesuryoHasuFlg;	-- 手数料端数計算フラグ
		v_item.l_inItem046 := gChohyoHankoFileNm;	-- 帳票印影の画像ファイルの絶対パス
		v_item.l_inItem047 := gInvoiceFlg;	-- オプションフラグ
		
		-- Call pkPrint.insertData with composite type
		CALL pkPrint.insertData(
			l_inKeyCd		=> l_inItakuKaishaCd,
			l_inUserId		=> l_inUserId,
			l_inChohyoKbn	=> l_inChohyoKbn,
			l_inSakuseiYmd	=> l_inGyomuYmd,
			l_inChohyoId	=> REPORT_ID,
			l_inSeqNo		=> gSeqNo,
			l_inHeaderFlg	=> '1',
			l_inItem		=> v_item,
			l_inKousinId	=> l_inUserId,
			l_inSakuseiId	=> l_inUserId
		);
	END LOOP;
	CLOSE curMeisai;
	IF pkControl.getOPTION_FLG(l_inItakuKaishaCd , GRS_OPTION_CD,'0') = '1' THEN
	    --「グロスアップ銘柄税率入力OP」の追加処理概要④
		--グロスアップ銘柄税率設定　最終ページ・最終行の処理
		--欄外表示の国税率リストの設定
		IF gSeqNo > 0 THEN	
			CALL SPIPI047K00R01_writeRangaiKokuZeiRateList(
				l_inItakuKaishaCd		,
				l_inUserId     			,
				l_inChohyoKbn  			,
				l_inGyomuYmd 			,
				REPORT_ID				,
				gSeqNo    				,
				gNo   					,
				grsRangaiKokuZeiRateList,
				grsRangaiChuki
			);
		END IF;
	END IF;
	IF gSeqNo = 0 THEN
		-- 対象データなし
		gRtnCd := RTN_NODATA;
		-- 帳票ワークへデータを追加
				-- Clear composite type
		v_item := ROW();
		
		v_item.l_inItem001 := gWrkTsuchiYmd;	-- 通知日
		v_item.l_inItem039 := '対象データなし';	-- 対象データ
		v_item.l_inItem047 := gInvoiceFlg;	-- オプションフラグ
		
		-- Call pkPrint.insertData with composite type
		CALL pkPrint.insertData(
			l_inKeyCd		=> l_inItakuKaishaCd,
			l_inUserId		=> l_inUserId,
			l_inChohyoKbn	=> l_inChohyoKbn,
			l_inSakuseiYmd	=> l_inGyomuYmd,
			l_inChohyoId	=> REPORT_ID,
			l_inSeqNo		=> 1,
			l_inHeaderFlg	=> '1',
			l_inItem		=> v_item,
			l_inKousinId	=> l_inUserId,
			l_inSakuseiId	=> l_inUserId
		);
	END IF;
	-- 終了処理
	l_outSqlCode := gRtnCd;
	l_outSqlErrM := '';
  -- エラー処理
EXCEPTION
	WHEN OTHERS THEN
		BEGIN
			CLOSE curMeisai;
		EXCEPTION
			WHEN OTHERS THEN
				NULL;
		END;
		CALL pkLog.fatal('ECM701', REPORT_ID, 'SQLCODE:' || SQLSTATE);
		CALL pkLog.fatal('ECM701', REPORT_ID, 'SQLERRM:' || SQLERRM);
		l_outSqlCode := RTN_FATAL;
		l_outSqlErrM := SQLERRM;
		--    RAISE;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi047k00r01 ( l_inItakuKaishaCd TEXT, l_inUserId TEXT, l_inChohyoKbn TEXT, l_inGyomuYmd TEXT, l_inHktCd TEXT, l_inKozaTenCd TEXT, l_inKozaTenCifCd TEXT, l_inMgrCd TEXT, l_inIsinCd TEXT, l_inKijunYm TEXT, l_inKijunYmdFrom TEXT, l_inKijunYmdTo TEXT, l_inTsuchiYmd TEXT, l_outSqlCode OUT numeric, l_outSqlErrM OUT text ) FROM PUBLIC;





CREATE OR REPLACE FUNCTION spipi047k00r01_createbun ( l_in_ReportID TEXT, l_in_PatternCd character(2)) RETURNS varchar AS $body$
DECLARE

-- 請求文章(ワーク)
aryBun pkIpaBun.BUN_ARRAY;
wkBun  varchar(200) := NULL;
BEGIN
-- 請求文章の取得
aryBun := pkIpaBun.getBun(l_in_ReportID, l_in_PatternCd);
FOR i IN 1 .. COALESCE(cardinality(aryBun), 0) LOOP
	-- 100byteまで全角スペース埋めして、請求文章を連結
	wkBun := wkBun || RPAD(aryBun[i], 100, '　');
END LOOP;
RETURN wkBun;
EXCEPTION
WHEN OTHERS THEN
	RAISE;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION spipi047k00r01_createbun ( l_in_ReportID TEXT, l_in_PatternCd character(2)) FROM PUBLIC;





CREATE OR REPLACE PROCEDURE spipi047k00r01_createsql (
	p_gRikinHasuFlg character varying,
	p_gTesuryoHasuFlg character varying,
	p_l_inChohyoKbn TEXT,
	p_l_inKijunYmdFrom TEXT,
	p_l_inKijunYmdTo TEXT,
	p_gGyomuYm char(6),
	p_gGnrshr0outFlg character varying,
	p_l_inItakuKaishaCd TEXT,
	p_l_inHktCd TEXT,
	p_l_inKozaTenCd TEXT,
	p_l_inKozaTenCifCd TEXT,
	p_l_inMgrCd TEXT,
	p_l_inIsinCd TEXT,
	p_gChohyoSortFlg character varying,
	p_outSQL OUT varchar
) AS $body$
DECLARE
	gSQL varchar(10000);
	gRikinHasuFlg character varying := p_gRikinHasuFlg;
	gTesuryoHasuFlg character varying := p_gTesuryoHasuFlg;
	l_inChohyoKbn TEXT := p_l_inChohyoKbn;
	l_inKijunYmdFrom TEXT := p_l_inKijunYmdFrom;
	l_inKijunYmdTo TEXT := p_l_inKijunYmdTo;
	gGyomuYm char(6) := p_gGyomuYm;
	gGnrshr0outFlg character varying := p_gGnrshr0outFlg;
	l_inItakuKaishaCd TEXT := p_l_inItakuKaishaCd;
	l_inHktCd TEXT := p_l_inHktCd;
	l_inKozaTenCd TEXT := p_l_inKozaTenCd;
	l_inKozaTenCifCd TEXT := p_l_inKozaTenCifCd;
	l_inMgrCd TEXT := p_l_inMgrCd;
	l_inIsinCd TEXT := p_l_inIsinCd;
	gChohyoSortFlg character varying := p_gChohyoSortFlg;
BEGIN
-- 変数を初期化
gSQL := '';
-- 変数にSQLクエリ文を代入
gSql :='SELECT'
	|| '		M01.HKT_CD,'
	|| '		M01.SFSK_POST_NO,'
	|| '		M01.ADD1,'
	|| '		M01.ADD2,'
	|| '		M01.ADD3,'
	|| '		M01.HKT_NM,'
	|| '		M01.SFSK_BUSHO_NM,'
	|| '		VJ1.BANK_NM,'
	|| '		VJ1.BUSHO_NM1,'
	|| '		VMG1.ISIN_CD,'
	|| '		VMG1.MGR_NM,'
	|| '		CASE'
	|| '			WHEN SUM(K01.GZEIHIKI_BEF_CHOKYU_KNGK) <> 0 AND'
	|| '				SUM(K01.SHOKAN_SEIKYU_KNGK) = 0 THEN'
	|| '			''利金'''
	|| '			WHEN SUM(K01.GZEIHIKI_BEF_CHOKYU_KNGK) = 0 AND'
	|| '				SUM(K01.SHOKAN_SEIKYU_KNGK) <> 0 THEN'
	|| '			''元金'''
	|| '			WHEN SUM(K01.GZEIHIKI_BEF_CHOKYU_KNGK) <> 0 AND'
	|| '				SUM(K01.SHOKAN_SEIKYU_KNGK) <> 0 THEN'
	|| '			''元利金'''
	|| '			ELSE'
	|| '				NULL'
	|| '		END AS GANRI_KBN,'
	|| '		K01.SHR_YMD,'
	|| '		SUM(K01.SHOKAN_SEIKYU_KNGK),'
	|| '		GM64.TSUKA_NM,'
	|| '		SUM(K01.GZEIHIKI_BEF_CHOKYU_KNGK),'
	|| '		RM64.TSUKA_NM,'
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--		|| '		M42.TAX_NM,'
--		|| '		M42.KOKU_ZEI_RATE,'
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	|| '		SUM(oracle.decode(K01.TAX_KBN, ''85'', 0::numeric, K01.GZEI_KNGK)),'
	|| '		RM64.TSUKA_NM,'
	|| '		SUM(oracle.decode(K01.TAX_KBN, ''85'', 0::numeric, K01.GZEIHIKI_AFT_CHOKYU_KNGK)),'
	|| '		RM64.TSUKA_NM,'
	|| '		CASE'
	|| '			WHEN VGK021.KKN_SHUKIN_KNGK > 0 THEN'
	|| '			MG8.GNKN_SHR_TESU_BUNSHI'
	|| '			ELSE'
	|| '			NULL'
	|| '		END AS GNKN_SHR_TESU_BUNSHI,'
	|| '		CASE'
	|| '			WHEN VGK021.KKN_SHUKIN_KNGK > 0 THEN'
	|| '			MG8.GNKN_SHR_TESU_BUNBO'
	|| '			ELSE'
	|| '			NULL'
	|| '		END AS GNKN_SHR_TESU_BUNBO,'
	|| '		VGK021.KKN_SHUKIN_KNGK,'
	|| '		CASE'
	|| '			WHEN VRK021.KKN_SHUKIN_KNGK > 0 THEN'
	|| '			MG8.RKN_SHR_TESU_BUNSHI'
	|| '			ELSE'
	|| '			NULL'
	|| '		END AS RKN_SHR_TESU_BUNSHI,'
	|| '		CASE'
	|| '			WHEN VRK021.KKN_SHUKIN_KNGK > 0 THEN'
	|| '			MG8.RKN_SHR_TESU_BUNBO'
	|| '			ELSE'
	|| '			NULL'
	|| '		END AS RKN_SHR_TESU_BUNBO,'
	|| '		VRK021.KKN_SHUKIN_KNGK,'
	|| '		VJ1.TESURYO_KOMI_FLG,'
	|| '		K01.MGR_CD,'
	|| '		K01.TAX_KBN,'
	|| '		M01.EIGYOTEN_CD,' -- IP-05983 営業店コード
	|| '		RPAD(M04.BUTEN_NM, 50, '' ''),'; -- IP-05983 営業店名称
IF gRikinHasuFlg = '0' THEN  -- 利金端数
	gSql := gSql || ' 0 AS RIKIN_HASU   ';
ELSE
	gSql := gSql || ' VRKH021.KKN_SHUKIN_KNGK AS RIKIN_HASU   ';
END IF;
	gSql := gSql || '	FROM KIKIN_SEIKYU K01,'
	|| '		MGR_TESURYO_PRM MG8,'
	|| '		MHAKKOTAI M01,'
	|| '		MBUTEN M04,' -- IP-05983 部店マスタ
	|| '		VJIKO_ITAKU VJ1,'
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--		|| '		MTAX M42,'
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	|| '		(SELECT *'
	|| '		  FROM MGR_KIHON_VIEW VMG12'
	|| '		 WHERE TRIM(VMG12.ISIN_CD) IS NOT NULL'
	|| '			AND VMG12.MGR_STAT_KBN = ''1'''
	|| '			AND VMG12.JTK_KBN <> ''2'''
	|| '			AND VMG12.JTK_KBN <> ''5'') VMG1,'
	|| '		MTSUKA GM64,'
	|| '		MTSUKA RM64,'
	|| '		(SELECT K021.ITAKU_KAISHA_CD,'
	|| '				K021.MGR_CD,'
	|| '				SUM(K021.KKN_SHUKIN_KNGK) AS KKN_SHUKIN_KNGK'
	|| '		  FROM KIKIN_IDO K021';
IF gTesuryoHasuFlg = '0' THEN
	gSql := gSql || '    WHERE K021.KKN_IDO_KBN IN (''32'', ''33'')';
ELSE
	gSql := gSql || '    WHERE K021.KKN_IDO_KBN IN (''32'', ''33'', ''94'')';
END IF;
  IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
   gSql := gSql || '    AND K021.RBR_YMD BETWEEN '''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo || ''' ';
  ELSE
   gSql := gSql || '    AND SUBSTR(K021.RBR_YMD, 1, 6) = '''|| gGyomuYm || '''';
  END IF;
  gSql :=  gSql || '		 GROUP BY K021.ITAKU_KAISHA_CD,'
	|| '				  K021.MGR_CD,'
	|| '				  SUBSTR(K021.RBR_YMD, 1, 6)) VGK021,'
	|| '		(SELECT K021.ITAKU_KAISHA_CD,'
	|| '				K021.MGR_CD,'
	|| '				SUM(K021.KKN_SHUKIN_KNGK) AS KKN_SHUKIN_KNGK'
	|| '		  FROM KIKIN_IDO K021';
IF gTesuryoHasuFlg = '0' THEN
	gSql :=  gSql || '		 WHERE K021.KKN_IDO_KBN IN (''42'', ''43'')';
ELSE
	gSql :=  gSql || '		 WHERE K021.KKN_IDO_KBN IN (''42'', ''43'', ''96'')';
END IF;
  IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
   gSql := gSql || '    AND K021.RBR_YMD BETWEEN '''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo || ''' ';
  ELSE
   gSql := gSql || '    AND SUBSTR(K021.RBR_YMD, 1, 6) = '''|| gGyomuYm || '''';
  END IF;
  gSql :=  gSql || '		 GROUP BY K021.ITAKU_KAISHA_CD,'
	|| '				  K021.MGR_CD,'
	|| '				  SUBSTR(K021.RBR_YMD, 1, 6)) VRK021,'
	|| '		(SELECT GK01.ITAKU_KAISHA_CD, GK01.MGR_CD'
	|| '		  FROM KIKIN_SEIKYU GK01';
  IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
   gSql := gSql || ' WHERE GK01.SHR_YMD BETWEEN'''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo || ''' ';
  ELSE
   gSql := gSql || ' WHERE SUBSTR(GK01.SHR_YMD, 1, 6) = '''|| gGyomuYm || '''';
  END IF;
  gSql :=  gSql || '		 GROUP BY GK01.ITAKU_KAISHA_CD, GK01.MGR_CD'
	|| '		HAVING SUM(GK01.SHOKAN_SEIKYU_KNGK) <> 0) VGK01,'
	|| '		(SELECT RK01.ITAKU_KAISHA_CD, RK01.MGR_CD'
	|| '		  FROM KIKIN_SEIKYU RK01';
  IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
   gSql := gSql || ' WHERE RK01.SHR_YMD BETWEEN'''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo || ''' ';
  ELSE
   gSql := gSql || ' WHERE SUBSTR(RK01.SHR_YMD, 1, 6) = '''|| gGyomuYm || '''';
  END IF;
  gSql :=  gSql	|| '		 GROUP BY RK01.ITAKU_KAISHA_CD, RK01.MGR_CD'
	|| '		HAVING SUM(RK01.GZEIHIKI_BEF_CHOKYU_KNGK) <> 0) VRK01,'
	|| '		(SELECT K021.ITAKU_KAISHA_CD,'
	|| '				K021.MGR_CD,'
	|| '				SUM(K021.KKN_SHUKIN_KNGK) AS KKN_SHUKIN_KNGK'
	|| '		  FROM KIKIN_IDO K021'
	|| '		 WHERE K021.KKN_IDO_KBN  = ''92''';
  IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
   gSql := gSql || '    AND K021.RBR_YMD BETWEEN '''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo || ''' ';
  ELSE
   gSql := gSql || '    AND SUBSTR(K021.RBR_YMD, 1, 6) = '''|| gGyomuYm || '''';
  END IF;
  gSql :=  gSql || '		 GROUP BY K021.ITAKU_KAISHA_CD,'
	|| '				  K021.MGR_CD,'
	|| '				  SUBSTR(K021.RBR_YMD, 1, 6)) VRKH021';
gSql :=  gSql || ' WHERE K01.ITAKU_KAISHA_CD = VMG1.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VMG1.MGR_CD'
	|| '	AND VMG1.ITAKU_KAISHA_CD = M01.ITAKU_KAISHA_CD'
	|| '	AND VMG1.HKT_CD = M01.HKT_CD'
	|| '	AND VMG1.SHOKAN_TSUKA_CD = GM64.TSUKA_CD'
	|| '	AND VMG1.RBR_TSUKA_CD = RM64.TSUKA_CD'
	|| '	AND K01.ITAKU_KAISHA_CD = VJ1.KAIIN_ID'
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--		|| '	AND K01.TAX_KBN = M42.TAX_KBN'
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
	|| '	AND K01.ITAKU_KAISHA_CD = MG8.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = MG8.MGR_CD'
	|| '	AND K01.ITAKU_KAISHA_CD = VGK021.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VGK021.MGR_CD'
	|| '	AND K01.ITAKU_KAISHA_CD = VRK021.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VRK021.MGR_CD'
-- 利金端数
	|| '	AND K01.ITAKU_KAISHA_CD = VRKH021.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VRKH021.MGR_CD'
	|| '	AND K01.ITAKU_KAISHA_CD = VGK01.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VGK01.MGR_CD'
	|| '	AND K01.ITAKU_KAISHA_CD = VRK01.ITAKU_KAISHA_CD'
	|| '	AND K01.MGR_CD = VRK01.MGR_CD'
--  IP-05983 営業店コード・名称表示対応
	|| '	AND M01.ITAKU_KAISHA_CD = M04.ITAKU_KAISHA_CD'
	|| '	AND M01.EIGYOTEN_CD = M04.BUTEN_CD'
-- 実質記番号方式は対象外
	|| '	AND K01.KK_KANYO_UMU_FLG <> ''2'' '
-- 機構非関与銘柄元利金請求データ(承認)で承認していない銘柄は対象外
	|| ' AND (K01.KK_KANYO_UMU_FLG = ''1'' '
	|| ' OR (K01.KK_KANYO_UMU_FLG = ''0'' AND K01.SHORI_KBN = ''1''))';
--  国税引前利金請求金額＋償還金請求金額＝０は作成対象外
IF gGnrshr0outFlg = '1' THEN
	gSql := gSql || ' AND K01.GZEIHIKI_BEF_CHOKYU_KNGK + K01.SHOKAN_SEIKYU_KNGK <> 0 ';
END IF;
-- 入力パラメータ条件
-- 委託会社コード
IF (trim(both l_inItakuKaishaCd) IS NOT NULL AND (trim(both l_inItakuKaishaCd))::text <> '') THEN
	gSql := gSql || '	AND K01.ITAKU_KAISHA_CD = ''' || l_inItakuKaishaCd ||
		''' ';
END IF;
-- 業務年月
IF l_inChohyoKbn = '0' AND (trim(both l_inKijunYmdFrom) IS NOT NULL AND (trim(both l_inKijunYmdFrom))::text <> '') THEN
	gSql := gSql || '	AND K01.SHR_YMD BETWEEN '''|| l_inKijunYmdFrom || ''' AND '''|| l_inKijunYmdTo ||
		''' ';
ELSIF (trim(both gGyomuYm) IS NOT NULL AND (trim(both gGyomuYm))::text <> '') THEN
	gSql := gSql || '	AND SUBSTR(K01.SHR_YMD,1, 6) = ''' || gGyomuYm ||
		''' ';
END IF;
-- リアルの場合
IF l_inChohyoKbn = '0' THEN
	-- 発行体コード
	IF (trim(both l_inHktCd) IS NOT NULL AND (trim(both l_inHktCd))::text <> '') THEN
		gSql := gSql || '	AND M01.HKT_CD = ''' || l_inHktCd || ''' ';
	END IF;
	-- 口座店コード
	IF (trim(both l_inKozaTenCd) IS NOT NULL AND (trim(both l_inKozaTenCd))::text <> '') THEN
		gSql := gSql || '	AND M01.KOZA_TEN_CD = ''' || l_inKozaTenCd ||
		        ''' ';
	END IF;
	-- 口座店CIFコード
	IF (trim(both l_inKozaTenCifCd) IS NOT NULL AND (trim(both l_inKozaTenCifCd))::text <> '') THEN
		gSql := gSql || '	AND M01.KOZA_TEN_CIFCD = ''' || l_inKozaTenCifCd ||
		        ''' ';
	END IF;
	-- 銘柄コード
	IF (trim(both l_inMgrCd) IS NOT NULL AND (trim(both l_inMgrCd))::text <> '') THEN
		gSql := gSql || '	AND VMG1.MGR_CD = ''' || l_inMgrCd || ''' ';
	END IF;
	-- ISINコード
	IF (trim(both l_inIsinCd) IS NOT NULL AND (trim(both l_inIsinCd))::text <> '') THEN
		gSql := gSql || '	AND VMG1.ISIN_CD = ''' || l_inIsinCd || ''' ';
	END IF;
END IF;
-- GROUP BY句
gSql := gSql	|| 'GROUP BY '
				|| '	K01.ITAKU_KAISHA_CD, '
				|| '	K01.MGR_CD, '
				|| '	K01.SHR_YMD, '
				|| '	K01.TAX_KBN, '
				|| '	M01.HKT_CD, '
				|| '	M01.SFSK_POST_NO, '
				|| '	M01.ADD1, '
				|| '	M01.ADD2, '
				|| '	M01.ADD3, '
				|| '	M01.HKT_NM, '
				|| '	M01.SFSK_BUSHO_NM, '
				|| '	M01.HKT_KANA_RNM, '					
				|| '	VJ1.BANK_NM, '
				|| '	VJ1.BUSHO_NM1, '
				|| '	VMG1.ISIN_CD, '
				|| '	VMG1.MGR_NM, '
-- 復興増税対応 - 2012/06/12 JSFIT山下 開始
--					|| '	M42.TAX_NM, '
--					|| '	M42.KOKU_ZEI_RATE, '
-- 復興増税対応 - 2012/06/12 JSFIT山下 終了
				|| '	VGK021.KKN_SHUKIN_KNGK, '
				|| '	VRK021.KKN_SHUKIN_KNGK, '
				|| '	VRKH021.KKN_SHUKIN_KNGK, '	-- 利金端数
				|| '	MG8.GNKN_SHR_TESU_BUNSHI, '
				|| '	MG8.GNKN_SHR_TESU_BUNBO, '
				|| '	MG8.RKN_SHR_TESU_BUNSHI, '
				|| '	MG8.RKN_SHR_TESU_BUNBO, '
				|| '	VJ1.TESURYO_KOMI_FLG, '
				|| '	GM64.TSUKA_NM, '
				|| '	RM64.TSUKA_NM, '
				|| '	M01.EIGYOTEN_CD, ' -- IP-05983 営業店コード
				|| '	M04.BUTEN_NM '; -- IP-05983 営業店名称
-- ORDER BY句
gSql := gSql || 'ORDER BY '
			 || '	K01.ITAKU_KAISHA_CD, '
			 || '	oracle.decode( (''' || gChohyoSortFlg || ''')::text , ''1'' , M01.HKT_KANA_RNM , M01.HKT_CD), '
			 || '	M01.HKT_CD, '
			 || '	oracle.decode( (''' || gChohyoSortFlg || ''')::text , ''1'' , K01.MGR_CD , VMG1.ISIN_CD), '
			 || '	K01.SHR_YMD, '
			 || '	GANRI_KBN, '
			 || '	K01.TAX_KBN ';
-- Return the generated SQL
p_outSQL := gSQL;
EXCEPTION
WHEN OTHERS THEN
	RAISE;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi047k00r01_createsql () FROM PUBLIC;





CREATE OR REPLACE PROCEDURE spipi047k00r01_getgrossupkokuzeiritsu ( l_inItakuKaishaCd TEXT , l_inMgrCd TEXT , l_inRbrYmd TEXT , l_inTaxKbn TEXT, l_outGrossKokuZeiRate OUT numeric(15,13) , l_outGrossTekiyoRbrKjt OUT character(8)) AS $body$
DECLARE

ora2pg_rowcount int;
wkRbrKjt  character(8);
wkKokuZeiRate  numeric(15,13)	:= NULL;
wkTekiyoRbrKjt character(8) := NULL;

BEGIN
	--「グロスアップ銘柄税率入力OP」の追加処理概要 ①	
	IF SPIPI047K00R01_isGrossupTaxKbn(l_inTaxKbn) THEN
	  --指定銘柄の利払日から利払期日を求める
	  SELECT
	    RBR_KJT
	  INTO STRICT wkRbrKjt
	  FROM
	  	MGR_RBRKIJ
	  WHERE
	  	ITAKU_KAISHA_CD = l_inItakuKaishaCd AND
	  	MGR_CD = l_inMgrCd AND
	  	RBR_YMD	= l_inRbrYmd;
	  	--指定銘柄のグロスアップ税率情報が取得利払期日より以前に存在するかチェックする。
		--MAX指定により直近の適用利払期日を取得する。
		GET DIAGNOSTICS ora2pg_rowcount = ROW_COUNT;
IF ora2pg_rowcount > 0 THEN
			--グロスアップ銘柄の指定利払日
			SELECT GRS_KOKU_ZEI_RATE, TEKIYO_RBR_KJT
			INTO STRICT  wkKokuZeiRate,wkTekiyoRbrKjt
  					FROM GROSSUP_MGR_TAX
			WHERE ITAKU_KAISHA_CD = l_inItakuKaishaCd AND MGR_CD = l_inMgrCd
   				AND TEKIYO_RBR_KJT
      				= (SELECT
          				MAX(trim(both TEKIYO_RBR_KJT))
          				FROM GROSSUP_MGR_TAX
           			WHERE ITAKU_KAISHA_CD = l_inItakuKaishaCd
             			AND MGR_CD =l_inMgrCd
             			AND TEKIYO_RBR_KJT <= wkRbrKjt
             			AND SHORI_KBN = '1'
           			GROUP BY ITAKU_KAISHA_CD, MGR_CD);
		END IF;
	END IF;
	l_outGrossKokuZeiRate := wkKokuZeiRate;
	l_outGrossTekiyoRbrKjt := wkTekiyoRbrKjt;
EXCEPTION
	WHEN no_data_found THEN
		l_outGrossKokuZeiRate := NULL;
		l_outGrossTekiyoRbrKjt := NULL;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi047k00r01_getgrossupkokuzeiritsu ( l_inItakuKaishaCd TEXT , l_inMgrCd TEXT , l_inRbrYmd TEXT , l_inTaxKbn TEXT, l_outGrossKokuZeiRate OUT numeric(15,13) , l_outGrossTekiyoRbrKjt OUT character(8)) FROM PUBLIC;



-- mTaxArray type removed - use simple array instead


CREATE OR REPLACE FUNCTION spipi047k00r01_isgrossuptaxkbn ( l_in_Taxkbn character(2)) RETURNS boolean AS $body$
DECLARE

  mta  character(2)[] := ARRAY['10', '20', '92', '94'];

BEGIN
  FOR i IN 1 .. array_length(mta, 1) LOOP
    IF l_in_TaxKbn = mta[i] THEN 
      RETURN TRUE;
    END IF;
  END LOOP;
  RETURN FALSE;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION spipi047k00r01_isgrossuptaxkbn ( l_in_Taxkbn MTAX.TAX_KBN%TYPE) FROM PUBLIC;





CREATE OR REPLACE PROCEDURE spipi047k00r01_updsreportwk ( l_inKeyCd SREPORT_WK.KEY_CD%TYPE, l_inUserId SREPORT_WK.USER_ID%TYPE, l_inChohyoKbn SREPORT_WK.CHOHYO_KBN%TYPE, l_inSakuseiYmd SREPORT_WK.SAKUSEI_YMD%TYPE, l_inChohyoId SREPORT_WK.CHOHYO_ID%TYPE, l_inSeqNo SREPORT_WK.SEQ_NO%TYPE, l_inRangaiKokuZeiRateList text, l_inRangaiChuki text ) AS $body$
BEGIN
	UPDATE SREPORT_WK
	SET ITEM041= l_inRangaiKokuZeiRateList
	   ,ITEM042= l_inRangaiChuki
	WHERE
		KEY_CD = l_inKeyCd AND
		USER_ID = l_inUserId AND
		CHOHYO_KBN = l_inChohyoKbn AND
		SAKUSEI_YMD = l_inSakuseiYmd AND
		CHOHYO_ID = l_inChohyoId AND
		SEQ_NO = l_inSeqNo;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi047k00r01_updsreportwk ( l_inKeyCd SREPORT_WK.KEY_CD%TYPE, l_inUserId SREPORT_WK.USER_ID%TYPE, l_inChohyoKbn SREPORT_WK.CHOHYO_KBN%TYPE, l_inSakuseiYmd SREPORT_WK.SAKUSEI_YMD%TYPE, l_inChohyoId SREPORT_WK.CHOHYO_ID%TYPE, l_inSeqNo SREPORT_WK.SEQ_NO%TYPE, l_inRangaiKokuZeiRateList text, l_inRangaiChuki text ) FROM PUBLIC;





CREATE OR REPLACE PROCEDURE spipi047k00r01_writerangaikokuzeiratelist ( l_inItakuKaishaCd SREPORT_WK.KEY_CD%TYPE,    -- 識別コード
 l_inUserId SREPORT_WK.USER_ID%TYPE,           -- ユーザID
 l_inChohyoKbn SREPORT_WK.CHOHYO_KBN%TYPE,        -- 帳票区分
 l_inGyomuYmd SREPORT_WK.SAKUSEI_YMD%TYPE,       -- 作成年月日
 l_inChohyoId SREPORT_WK.CHOHYO_ID%TYPE,         -- 帳票ＩＤ         
 l_inSeqNo SREPORT_WK.SEQ_NO%TYPE,            -- 連番
 l_inNo integer ,                          -- No
 l_inRangaiKokuZeiRateList text,    -- 欄外表示の国税率リスト
 l_inRangaiChuki text 			-- 欄外注記の文言
 ) AS $body$
DECLARE

  wkGrsNo    integer := 0; 						-- グロスアップ税率設定NO
BEGIN
		--元利金支払報告書帳票の欄外税率リスト設定行を導く
            --各ページの末尾のレコードに「欄外表示の国税率リスト」を更新する。
		
		--更新対象末尾レコード連番 = 帳票ワークの連番 - ブレイク毎のカウント数 + 1ページのＭＡＸ詳細レコード数
		wkGrsNo := l_inSeqNo - l_inNo + ROW_MAX;	
		LOOP
			IF wkGrsNo >= l_inSeqNo THEN
			    --端数レコードの国税率リスト設定
				CALL updSreportWK(
				  l_inItakuKaishaCd 						-- 識別コード
				  ,l_inUserId 							-- ユーザＩＤ
				  ,l_inChohyoKbn 						-- 帳票区分
				  ,l_inGyomuYmd 							-- 作成年月日
				  ,REPORT_ID 							-- 帳票ＩＤ
				  ,l_inSeqNo 							-- 末尾のレコード連番　端数レコードはブレイク時の帳票ワーク連番で更新
				  ,grsRangaiKokuZeiRateList              -- 欄外表示の国税率リスト
				  ,grsRangaiChuki                        -- 欄外注記の文言
				);
				EXIT;
			END IF;
			CALL updSreportWK(
				  l_inItakuKaishaCd 						-- 識別コード
				  ,l_inUserId 							-- ユーザＩＤ
				  ,l_inChohyoKbn 						-- 帳票区分
				  ,l_inGyomuYmd 							-- 作成年月日
				  ,REPORT_ID 							-- 帳票ＩＤ
				  ,wkGrsNo  								-- 末尾のレコード連番
				  ,grsRangaiKokuZeiRateList              -- 欄外表示の国税率リスト
				  ,grsRangaiChuki                        -- 欄外注記の文言
			);
			--更新対象末尾レコード連番 =  ページ毎末尾レコード連番 + 1ページのＭＡＸ詳細レコード数
			wkGrsNo := wkGrsNo + ROW_MAX;	
		END LOOP;
END;
$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi047k00r01_writerangaikokuzeiratelist ( l_inItakuKaishaCd SREPORT_WK.KEY_CD%TYPE, l_inUserId SREPORT_WK.USER_ID%TYPE, l_inChohyoKbn SREPORT_WK.CHOHYO_KBN%TYPE, l_inGyomuYmd SREPORT_WK.SAKUSEI_YMD%TYPE, l_inChohyoId SREPORT_WK.CHOHYO_ID%TYPE, l_inSeqNo SREPORT_WK.SEQ_NO%TYPE, l_inNo integer , l_inRangaiKokuZeiRateList text, l_inRangaiChuki text ) FROM PUBLIC;
