...
CREATE OR REPLACE FUNCTION sfadi017s05119 ( l_inKkSakuseiDt KK_RENKEI.KK_SAKUSEI_DT%TYPE, l_inDenbunMeisaiNo KK_RENKEI.DENBUN_MEISAI_NO%TYPE ) RETURNS integer AS $body$
DECLARE
...
-- *====================================================================
	-- 本SPのID
	SP_ID				CONSTANT text := 'SFADI017S05119';
	-- ユーザID
	USER_ID				CONSTANT text := pkconstant.BATCH_USER();
	-- 帳票ID
	REPORT_ID			CONSTANT text := '';
    -- 機構ステータス
    UPDATE_KK_STAT      CONSTANT text  := pkKkNotice.MGR_KKSTAT_SHONIN();
--====================================================================*
--   メイン
-- *====================================================================
BEGIN
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION sfadi017s05119 ( l_inKkSakuseiDt KK_RENKEI.KK_SAKUSEI_DT%TYPE, l_inDenbunMeisaiNo KK_RENKEI.DENBUN_MEISAI_NO%TYPE ) FROM PUBLIC;