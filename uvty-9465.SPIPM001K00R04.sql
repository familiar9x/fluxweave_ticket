...
CREATE OR REPLACE PROCEDURE spipm001k00r04 ( l_inItakuKaishaCd SREPORT_WK.KEY_CD%TYPE,     -- 委託会社コード
...
-- * 会社名:JIP
-- *
-- * 概要　:備忘録マスタ画面より、銘柄連動型の備忘録コードの時、備忘録コード別銘柄情報一覧表を作成する
-- *
...
-- *
-- * @author ASK
-- * @version $Id: SPIPM001K00R04.sql,v 1.2 2006/08/14 10:02:23 yamaki Exp $
-- *
-- ***************************************************************************
-- * ログ　:
-- * 　　　日付  開発者名    目的
...
--
	...
	--                  定数定義                                                    
	--==============================================================================
	C_PROCEDURE_ID CONSTANT text := 'SPIPM001K00R04';            -- プロシージャＩＤ
	C_CHOHYO_ID    CONSTANT SREPORT_WK.CHOHYO_ID%TYPE  := 'IPM30000131'; -- 帳票ＩＤ
	--==============================================================================
	--                  変数定義                                                    
	--==============================================================================
	...
	curMeisai CURSOR FOR
		SELECT
			ME05.MEMORANDOM_CD,                           -- 備忘録コード
			ME01.MEMORANDOM_TYPE_CD,                      -- 備忘録設定タイプコード
			ME02.MEMORANDOM_TYPE_NM,                      -- 備忘録設定タイプ名称
			ME01.HYOJI_BUNSHO,                            -- 表示文章
			ME05.MGR_CD,                                  -- 銘柄コード
			VMG1.ISIN_CD,                                  -- ＩＳＩＮコード
			VMG1.MGR_RNM,                                  -- 銘柄略称
			ME05.MEMORANDOM_BIKO                           -- 備忘録備考
		FROM (SELECT	DISTINCT
					ME05.ITAKU_KAISHA_CD,
					ME05.MGR_CD,
					ME05.MEMORANDOM_CD,
					ME05.MEMORANDOM_BIKO
			FROM	MEMORANDOM_M_SAGYO ME05
			WHERE	ME05.ITAKU_KAISHA_CD = l_inItakuKaishaCd
			AND		ME05.MEMORANDOM_CD = l_inMemorandomCd) ME05,
			MMEMORANDOM ME01,
		...
		WHERE
			...
			AND ME05.ITAKU_KAISHA_CD = VMG1.ITAKU_KAISHA_CD
			AND ME05.MGR_CD = VMG1.MGR_CD
			AND VMG1.MGR_STAT_KBN = '1'
			AND ME01.MEMORANDOM_TYPE_CD = ME02.MEMORANDOM_TYPE_CD
			AND ME02.MGR_RENDO_FLG = '1'
		ORDER BY
			-- 備忘録コード,銘柄コード
			ME05.MEMORANDOM_CD,
			ME05.MGR_CD;
...
BEGIN
	...
	LOOP
		...
		v_item.l_inItem003 := recMeisai.MEMORANDOM_TYPE_CD;	-- 備忘録タイプコード
		v_item.l_inItem004 := recMeisai.MEMORANDOM_TYPE_NM;	-- 備忘録タイプ名称
		v_item.l_inItem005 := recMeisai.HYOJI_BUNSHO;	-- 表示文章
		v_item.l_inItem006 := gSeqNo;	-- Ｎｏ（４桁）連番
		...
		v_item.l_inItem010 := recMeisai.MEMORANDOM_BIKO;	-- 備忘録備考
		v_item.l_inItem011 := C_CHOHYO_ID;	-- 帳票ＩＤ
		v_item.l_inItem012 := l_inUserId;	-- ユーザーＩＤ
	...
	IF gSeqNo = 1 THEN
		...
		v_item.l_inItem001 := gItakuKaishaRnm;	-- 委託会社略称
		v_item.l_inItem002 := l_inMemorandomCd;	-- 備忘録コード
		v_item.l_inItem011 := C_CHOHYO_ID;	-- 帳票ＩＤ
		v_item.l_inItem012 := l_inUserId;	-- ユーザーＩＤ
		v_item.l_inItem013 := '対象データ無し';	-- 対象データ無し
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipm001k00r04 ( l_inItakuKaishaCd SREPORT_WK.KEY_CD%TYPE, l_inUserId SREPORT_WK.USER_ID%TYPE, l_inChohyoKbn SREPORT_WK.CHOHYO_KBN%TYPE, l_inMemorandomCd text, l_outSqlCode OUT numeric, l_outSqlErrM OUT text ) FROM PUBLIC;