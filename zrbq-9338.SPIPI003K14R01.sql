...
CREATE OR REPLACE PROCEDURE spipi003k14r01 ( l_inItakuKaishaCd text,    -- 委託会社コード
...
-- 
  ...
  CHOHYOID            CONSTANT varchar(20) := 'IPQ30000311';            --固定値．帳票ID
...
--１−４．引数担保区分判定
 ...
 RAISE NOTICE '[DEBUG] Checkpoint 1: insertHeader before, gBaYm=%', gBaYm;
--２．ヘッダーレコード出力
 ...
 RAISE NOTICE '[DEBUG] Checkpoint 2: insertHeader done';
--３．未収前受収益計算処理実行
 ...
 RAISE NOTICE '[DEBUG] Checkpoint 3: calcHosei done, gRet=%', gRet;
 IF gRet <> 0 THEN
   RAISE NOTICE '[DEBUG] ERROR: calcHosei returned %, exiting', gRet;
   l_outSqlCode := gRet;
   RETURN;
 END IF;
--４．帳票ワークテーブルへ出力
 ...
 --４−１−５．レコードのINSERT処理を行う
  CALL pkPrint.insertData(
            ...
            ,l_inChohyoId     =>        CHOHYOID                           -- 帳票ＩＤ
            ,l_inSeqNo        =>        gCount                               -- 連番
            ,l_inHeaderFlg  =>      '1'                                   -- ヘッダーフラグ
            ,l_inItem001      =>        l_inBaYmd                            -- 引数．基準年度
            ,l_inItem002      =>        gPlaCls                          -- 期別
            ,l_inItem003      =>        reMeisai.FB                          -- 明細データ抽出．手数料分類
            ,l_inItem004      =>        reMeisai.MISHU                   -- 明細データ抽出．未既収区分
            ,l_inItem005      =>        reMeisai.ISIN                        -- 明細データ抽出．ISINコード
            ,l_inItem006      =>        reMeisai.MGR                         -- 明細データ抽出．銘柄コード
            ,l_inItem007      =>        reMeisai.RNM                         -- 明細データ抽出．銘柄略称
            ,l_inItem008        =>    reMeisai.ZNDK              -- 明細データ抽出．基準残高
            ,l_inItem009        =>    gTesu                    -- 変数．手数料率
            ,l_inItem010        =>    reMeisai.SRN             -- 明細データ抽出．手数料種類名称
            ,l_inItem011        =>    reMeisai.NYU             --明細データ抽出．入金日
            ,l_inItem012        =>    reMeisai.MI              --明細データ抽出．受託区分
            ,l_inItem013        =>    reMeisai.HBBS            --明細データ抽出．補正期間分母開始日
            ,l_inItem014        =>    reMeisai.HBBE            --明細データ抽出．補正期間分母終了日
            ,l_inItem015        =>    gDateCnt                 --変数．未既収日数
            ,l_inItem016        =>    gBiCnt                   --変数．期間日数
            ,l_inItem017        =>    reMeisai.BUNS            --明細データ抽出．自行分配率(分子)
            ,l_inItem018        =>    reMeisai.BUNB            --明細データ抽出．自行分配率(分母)
            ,l_inItem019        =>    reMeisai.HTNK            --明細データ抽出．補正_手数料額（税抜)
            ,l_inItem020        =>    reMeisai.HSK             --明細データ抽出．補正_消費税額
            ,l_inItem021        =>    CHOHYOID                        --固定値．帳票ID
            ,l_inItem022        =>    reMeisai.CPC                    --明細データ抽出．計算パターン
            ,l_inItem023        =>      gTitleCls                       -- タイトル　分類
            ,l_inItem024        =>      l_inUserId                                -- ユーザＩＤ
            ,l_inKousinId   =>    l_inUserId                      --引数．ユーザID
            ,l_inSakuseiId  =>    l_inUserId                      --引数．ユーザID
            );
  END LOOP;
 ELSE
  CALL pkPrint.insertHeader(l_inItakuKaishaCd,l_inUserId,l_inChohyoKbn,l_inGyomuYmd,CHOHYOID);
 ...
 IF gCount = 0 THEN
  RAISE NOTICE '[DEBUG] Checkpoint 4: No data found, gCount=0';
  CALL pkPrint.insertData(
            l_inKeyCd           =>      l_inItakuKaishaCd                -- 識別コード
            ,l_inUserId       =>        l_inUserId                         -- ユーザＩＤ
            ,l_inChohyoKbn  =>      l_inChohyoKbn                      -- 帳票区分
  ...
  RAISE NOTICE '[DEBUG] Returning l_outSqlCode=2 (No Data)';
 ELSE
  --リターン値＝0：正常終了を戻す。
  RAISE NOTICE '[DEBUG] Checkpoint 5: Data found, gCount=%, returning gRtnCd=%', gCount, gRtnCd;
  l_outSqlCode := gRtnCd;
  l_outSqlErrM := '';
 END IF;
 --処理終了
