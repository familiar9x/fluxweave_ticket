...
CREATE OR REPLACE PROCEDURE spip00802 ( l_inKessaiYmdF TEXT, -- 決済日(From)
...
--  /* 会社名:JIP
--  /*
--  /* 概要　:新規記録資金入金予定表出力指示画面またはバッチ起動の入力条件により、
--  /* 　　　 新規記録資金入金予定表（資金決済会社別）を作成する。
--  /*
...
--  /*
--  /* 返り値:なし
--  /* @version $Id: SPIP00802.SQL,v 1.17 2006/12/13 07:10:11 iwakami Exp $
--  /*
--  ***************************************************************************
--  /* ログ　:
--  /* 　　　日付  開発者名    目的
...
--  
	...
	REPORT_ID CONSTANT char(11) := 'IP030000821'; -- 帳票ID
	-- 書式フォーマット
	...
	gCnt                integer := 0; -- 合計件数
	gDvpKbnCnt          integer := 0; -- ＤＶＰ区分ごとの件数
	gSknKessaiMethodCnt integer := 0; -- 資金決済方法名称ごとの件数
	gSknKessaiKaishaCnt integer := 0; -- 資金決済会社コードごとの件数
	-- 退避用フィールド
	beforeKessaiYmd       varchar(8) := NULL; -- 決済日
	beforeHakkoTsukaCd    varchar(3) := NULL; -- 発行通貨コード
	beforeDvpKbn          varchar(8) := NULL; -- ＤＶＰ区分名称
	beforeSknKessaiMethod varchar(60) := NULL; -- 資金決済方法名称
	beforeSknKessaiKaisha varchar(7) := NULL; -- 資金決済会社コード
	-- 決済日がNULLの時の処理用変数
  ...
  gJikoDaikoKbn   VJIKO_ITAKU.JIKO_DAIKO_KBN%TYPE;    -- 自行代行区分
	...
	curMeisai CURSOR FOR
		SELECT B03.KESSAI_YMD, -- 決済日
					 M64.TSUKA_NM AS HAKKO_TSUKA_NM, -- 発行通貨
					 ...
					 MG1.MGR_CD, -- 銘柄コード
					 MG1.MGR_RNM, -- 銘柄略称
					 B03.AITE_KKMEMBER_FS_KBN || AITE_KKMEMBER_BCD ||
					 B03.AITE_KKMEMBER_KKBN AS KKMEMBER_CD, -- 機構加入者コード
					 M021.BANK_RNM AS KKMEMBER_RNM, -- 相手側機構加入者略称
					 ...
					 MCD2.CODE_RNM AS SKN_KESSAI_METHOD_NM, -- 資金決済方法名称
					 MCD3.CODE_NM AS DAIRI_MOTION_FLG_NM, -- 代理人直接申請フラグ名称
				--	 VJ1.SHINKI_KESSAI_JIGEN, -- 新規記録用決済時限
					 CASE WHEN B03.DVP_KBN='1' THEN B03.KESSAI_JIGEN  ELSE ' ' END  AS SHINKI_KESSAI_JIGEN, -- 新規記録用決済時限
					 VJ1.BANK_RNM, -- 銀行略称
...
WHERE B03.KESSAI_YMD BETWEEN gKessaiYmdF AND gKessaiYmdT AND B03.MGR_CD = MG1.MGR_CD AND B03.ITAKU_KAISHA_CD = MG1.ITAKU_KAISHA_CD AND VJ1.KAIIN_ID = l_inItakuKaishaCd AND VJ1.KAIIN_ID = MG1.ITAKU_KAISHA_CD          AND MG0.ITAKU_KAISHA_CD = MG1.ITAKU_KAISHA_CD AND MG0.MGR_CD = MG1.MGR_CD AND MG0.MGR_STAT_KBN = '1' AND MG0.MASSHO_FLG = '0'
		ORDER  BY B03.KESSAI_YMD,
							MG1.HAKKO_TSUKA_CD,
							B03.DVP_KBN,
							B03.SKN_KESSAI_METHOD_CD,
...
BEGIN
	IF DEBUG = 1
	THEN
		CALL pkLog.debug(l_inUserId, REPORT_ID, 'spIp00802 START');
	END IF;
	-- 入力パラメータのチェック
	...
	END IF;
	-- その他の入力パラメータのチェック
	IF coalesce(trim(both l_inItakuKaishaCd)::text, '') = '' OR
	...
	LOOP
		-- 各件数のカウントと初期化
		IF beforeKessaiYmd = recMeisai.KESSAI_YMD  -- 決済日
		THEN
			IF beforeHakkoTsukaCd = recMeisai.HAKKO_TSUKA_NM  -- 発行通貨
			THEN
				gCnt := gCnt + 1;
				IF beforeDvpKbn = recMeisai.DVP_KBN_NM  -- ＤＶＰ区分名称
				THEN
					gDvpKbnCnt := gDvpKbnCnt + 1;
					IF beforeSknKessaiMethod = recMeisai.SKN_KESSAI_METHOD_NM  -- 資金決済方法名称
					THEN
						gSknKessaiMethodCnt := gSknKessaiMethodCnt + 1;
						IF beforeSknKessaiKaisha = recMeisai.SKN_KESSAI_KAISHA_CD  -- 資金決済会社コード
						THEN
							gSknKessaiKaishaCnt := gSknKessaiKaishaCnt + 1;
						ELSE
							gSknKessaiKaishaCnt := 0;
						END IF;
					ELSE
						gSknKessaiMethodCnt := 0;
						gSknKessaiKaishaCnt := 0;
					END IF;
				ELSE
					gDvpKbnCnt          := 0;
		...
		END IF;
		-- 項目の退避
		beforeKessaiYmd       := recMeisai.KESSAI_YMD; -- 決済日
		beforeHakkoTsukaCd    := recMeisai.HAKKO_TSUKA_NM; -- 発行通貨
		beforeDvpKbn          := recMeisai.DVP_KBN_NM; -- ＤＶＰ区分名称
		beforeSknKessaiMethod := recMeisai.SKN_KESSAI_METHOD_NM; -- 資金決済方法名称
		beforeSknKessaiKaisha := recMeisai.SKN_KESSAI_KAISHA_CD; -- 資金決済会社コード
		-- シーケンスのカウント
		gSeqNo := gSeqNo + 1;
		-- 書式フォーマットの設定
		-- 発行
		...
		v_item.l_inItem004 := recMeisai.SKN_KESSAI_METHOD_NM;	-- 資金決済方法名称
		v_item.l_inItem005 := recMeisai.HAKKO_TSUKA_NM;	-- 発行通貨
		v_item.l_inItem006 := recMeisai.SKN_KESSAI_KAISHA_CD;	-- 資金決済会社コード
		v_item.l_inItem007 := recMeisai.SKN_KESSAI_KAISHA_RNM;	-- 相手側資金決済会社略称
		v_item.l_inItem008 := recMeisai.KESSAI_NO;	-- 決済番号
		v_item.l_inItem009 := recMeisai.KESSAI_KNGK;	-- 決済金額
		v_item.l_inItem010 := recMeisai.KKMEMBER_CD;	-- 機構加入者コード
		v_item.l_inItem011 := recMeisai.KKMEMBER_RNM;	-- 相手側機構加入者略称
		v_item.l_inItem012 := recMeisai.ISIN_CD;	-- ＩＳＩＮコード
		v_item.l_inItem013 := recMeisai.MGR_CD;	-- 銘柄コード
		v_item.l_inItem014 := recMeisai.MGR_RNM;	-- 銘柄略称
		v_item.l_inItem015 := recMeisai.DAIRI_MOTION_FLG_NM;	-- 代理人直接申請フラグ名称
		v_item.l_inItem016 := recMeisai.SHINKI_KESSAI_JIGEN;	-- 決済時限
		v_item.l_inItem017 := gItakuKaishaRnm;	-- 会社委託略称
		v_item.l_inItem018 := REPORT_ID;	-- 帳票ID
		...
		v_item.l_inItem023 := l_inGyomuYmd;	-- 作成日
		v_item.l_inItem024 := gCnt + 1;	-- 合計件数
		v_item.l_inItem025 := gDvpKbnCnt + 1;	-- ＤＶＰ区分ごとの件数
		v_item.l_inItem026 := gSknKessaiMethodCnt + 1;	-- 資金決済方法名称ごとの件数
		v_item.l_inItem027 := gSknKessaiKaishaCnt + 1;	-- 資金決済会社コードごとの件数
	...
	THEN
		...
		v_item.l_inItem017 := gItakuKaishaRnm;	-- 会社委託略称
		v_item.l_inItem018 := REPORT_ID;
		v_item.l_inItem019 := FMT_HAKKO_KNGK_J;
		v_item.l_inItem020 := FMT_RBR_KNGK_J;
		v_item.l_inItem021 := FMT_SHOKAN_KNGK_J;
	...
	IF DEBUG = 1
	THEN
		CALL pkLog.debug(l_inUserId, REPORT_ID, 'spIp00802 END');
	END IF;
	-- エラー処理
EXCEPTION
	WHEN OTHERS THEN
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spip00802 ( l_inKessaiYmdF TEXT, l_inKessaiYmdT TEXT, l_inItakuKaishaCd TEXT, l_inUserId TEXT, l_inChohyoKbn TEXT, l_inGyomuYmd TEXT, l_outSqlCode OUT integer, l_outSqlErrM OUT text ) FROM PUBLIC;