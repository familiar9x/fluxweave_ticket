...
CREATE OR REPLACE PROCEDURE sfipf001k00r02 ( l_inItakuKaishaCd TEXT,		-- 委託会社コード
...
--==============================================================================
	...
	REPORT_ID			CONSTANT char(11)	:= 'IP030000111';			-- 帳票ID
--==============================================================================
--					変数定義													
--==============================================================================
...
--==============================================================================
	curMeisai CURSOR FOR
		...
		AND		MCD1.CODE_SHUBETSU = '511'
		AND		MCD1.CODE_VALUE = M01.KOBETSU_SHONIN_SAIYO_FLG
		AND		MCD2.CODE_SHUBETSU = '705'
		AND		MCD2.CODE_VALUE = M01.GYOSHU_CD
		AND		MCD3.CODE_SHUBETSU = '717'
		AND		MCD3.CODE_VALUE = M01.RYOSHU_OUT_KBN
		AND		MCD4.CODE_SHUBETSU = '122'
		AND		MCD4.CODE_VALUE = M01.NYUKIN_KOZA_KBN
		AND		MCD5.CODE_SHUBETSU = '707'
		AND		MCD5.CODE_VALUE = M01.BD_KOZA_KAMOKU_CD
		AND		MCD6.CODE_SHUBETSU = '707'
		AND		MCD6.CODE_VALUE = M01.HKT_KOZA_KAMOKU_CD
		AND		MCD7.CODE_SHUBETSU = '711'
		AND		MCD7.CODE_VALUE = M01.HIKIOTOSHI_FLG
		AND		MCD8.CODE_SHUBETSU = '707'
		AND		MCD8.CODE_VALUE = M01.HKO_KAMOKU_CD
		AND		to_char(M01.SAKUSEI_DT, 'yyyymmdd') = l_inGyomuYmd
		AND		M01.SAKUSEI_ID = l_inUserId
		ORDER BY M01.HKT_CD;
	curCount CURSOR FOR
		SELECT 	COUNT(*) AS CNT
		FROM	PRT_OK	S05
		WHERE	trim(both S05.ITAKU_KAISHA_CD) = trim(both l_inItakuKaishaCd)
		AND		trim(both S05.KIJUN_YMD) = trim(both l_inGyomuYmd)
		AND		trim(both S05.LIST_SAKUSEI_KBN) = trim(both l_inChohyoSakuKbn)
		AND		trim(both S05.CHOHYO_ID) = trim(both REPORT_ID);
...
--	制作時、ログを出力する。パッケージ使用		
	IF DEBUG = 1 THEN	
	CALL pkLog.debug(l_inUserId, REPORT_ID, 'SFIPF001K00R02 START');	
	END IF;
	...
	FOR recMeisai IN curMeisai LOOP
		...
		v_item.l_inItem049 := recMeisai.HIKIOTOSHI_FLG;	-- 自動引落フラグ
		v_item.l_inItem050 := recMeisai.HF_NM;	-- 自動引落フラグ内容
		v_item.l_inItem051 := recMeisai.HKO_KAMOKU_CD;	-- 自動引落口座_口座科目コード
		v_item.l_inItem052 := recMeisai.HKC_NM;	-- 自動引落口座_口座科目名称
		v_item.l_inItem053 := recMeisai.HKO_KOZA_NO;	-- 自動引落口座_口座番号
		v_item.l_inItem054 := recMeisai.HKO_KOZA_MEIGININ_NM;	-- 自動引落口座_口座名義人
		v_item.l_inItem055 := recMeisai.HKO_KOZA_MEIGININ_KANA_NM;	-- 自動引落口座_口座名義人(カナ)
		v_item.l_inItem056 := recMeisai.DEFAULT_YMD;	-- デフォルト日
		v_item.l_inItem057 := recMeisai.DEFAULT_BIKO;	-- デフォルト備考
		v_item.l_inItem058 := recMeisai.YOBI1;	-- 予備１
		v_item.l_inItem059 := recMeisai.YOBI2;	-- 予備２
		v_item.l_inItem060 := recMeisai.YOBI3;	-- 予備３
	...
	IF DEBUG = 1 THEN	CALL pkLog.debug(l_inUserId, REPORT_ID, 'SFIPF001K00R02 END');	END IF;
-- エラー処理
EXCEPTION
	WHEN	OTHERS THEN
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE sfipf001k00r02 ( l_inItakuKaishaCd TEXT, l_inUserId TEXT, l_inChohyoKbn TEXT, l_inChohyoSakuKbn TEXT, l_inGyomuYmd TEXT, l_outSqlCode OUT integer, l_outSqlErrM OUT text ) FROM PUBLIC;