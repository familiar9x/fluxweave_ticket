...
DROP TYPE IF EXISTS spip03504_01_type_record CASCADE;
CREATE TYPE spip03504_01_type_record AS (
		...
		,gSfmtBankNm			varchar(70) 			-- 送付元金融機関名称
...
DROP TYPE IF EXISTS spip03504_01_type_key CASCADE;
CREATE TYPE spip03504_01_type_key AS (
		gBankCd					char(4)		-- 金融機関コード
		,gDistriYmd				char(8)		-- 分配日
		,gTsukaCd				char(3)		-- 通貨コード
	);
DROP TYPE IF EXISTS spip03504_01_type_gokei CASCADE;
...
EXCEPTION
	WHEN	OTHERS	THEN
		-- Close cursor if it exists (PostgreSQL doesn't have %ISOPEN)
		BEGIN
			CLOSE curMeisai;
		EXCEPTION WHEN OTHERS THEN
			NULL; -- Ignore if cursor is not open
		END;
