...
CREATE OR REPLACE FUNCTION sfip931500141 ( l_inUserId text        -- ユーザID
...
--==============================================================================
  ...
  PROGRAM_ID    CONSTANT varchar(30) := 'SFIP931500141';   -- プログラムＩＤ
--=======================================================================================================*
-- * 変数定義
-- *=======================================================================================================
...
BEGIN
  ...
  END IF;
  --  アップフロント分伝票起票シートを出力  
  SELECT f.l_outerrmsg, f.extra_param
  INTO l_outErrMsg, returnCd
  FROM SFIP931500141_01(
      l_inUserId         -- ユーザID
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION sfip931500141 ( l_inUserId text , l_inItakuKaishaCd text , l_inMeigaraCode text , l_inIsinCd text , l_inKijun_Ym text , l_inKeijo_Ymd text , l_inChohyo_Kbn text , l_outErrMsg OUT text , OUT extra_param numeric) FROM PUBLIC;
