SQL (PLSQL): plsql/ipa/option/Kounai/ShokiIkou/SPIPF003K00R02.sql
...
If you needed to convert type (e.g. NUMERIC to NUMERIC), please make sure to convert the type correctly. NUMERIC should be converted to NUMERIC (not INT, BIGINT, etc).
There will be a lot of mixed types usage which is fined in Oracle but will not work in Postgres. For this, cast the type so that the expected type in the function work correctly (e.g. someText = 0 works in Oracle, but failed in Postgres, so to fix this, do someText = '0'). DO NOT change the type of the function (you may change type TEXT to VARCHAR if needed, but not CHAR(3) to VARCHAR for example). The reason that you can't change the type of the function is because the functions can be called at multiple places so if you updated the function, other places will failed.
...
Generated: generated/db_output_sql/main/plsql/ipa/option/Kounai/ShokiIkou/SPIPF003K00R02.sql
Legacy: legacy/db/plsql/ipa/option/Kounai/ShokiIkou/SPIPF003K00R02.sql
Save to: db/plsql/ipa/option/Kounai/ShokiIkou/SPIPF003K00R02.sql
Example
