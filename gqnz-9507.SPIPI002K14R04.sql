...
CREATE OR REPLACE PROCEDURE spipi002k14r04 ( 
...
-- *
-- * @author Xu Chunxu
-- * @version $Id: SPIPI002K14R04.sql,v 1.2 2013/06/19 05:54:01 ito Exp $
--
	--==============================================================================
	--                 デバッグ゛機能                                                
	--==============================================================================
	...
	--                  定数定義                                                    
	--==============================================================================
	REPORT_ID        CONSTANT text       := 'IP931400241';    -- 帳票ＩＤ
	PROGRAM_ID     CONSTANT text         := 'SPIPI002K14R04';  -- SP名称
	PROGRAM_NAME      CONSTANT text     := '現物債未払管理入力取込確認リスト（記番号データ）';  -- 機能名称
	--==============================================================================
	--                  変数定義                                                    
	--==============================================================================
	v_item type_sreport_wk_item;						-- Composite type for pkPrint.insertData
    gRtnCd          numeric := pkconstant.success();      -- リターンコード
	errUmuFlg       numeric := 9;                       --エラーフラグ
...
BEGIN
	...
	FOR recCount IN curCount LOOP
		...
		v_item.l_inItem008 := pkcharacter.numeric_to_char(errCnt);	-- 件数
		...
		ELSE
			FOR recResult IN curResult LOOP
				...
				ELSIF recResult.ERR_UMU_FLG = '0' THEN
  					IF errUmuFlg::text <> recResult.ERR_UMU_FLG THEN
								-- Clear composite type
		v_item := ROW();
		...
		v_item.l_inItem001 := l_inUserId;	-- ユーザＩＤ
		v_item.l_inItem002 := pkcharacter.numeric_to_char(gNo);	-- 番号
		v_item.l_inItem003 := REPORT_ID;	-- 帳票ＩＤ
		...
		CALL pkPrint.insertData(
			...
			l_inSakuseiId	=> l_inUserId
				...
				errUmuFlg := recResult.ERR_UMU_FLG::numeric;
			END LOOP;
		END IF;
		IF errUmuFlg = 1 THEN
			gSeqNo := gSeqNo + 1;
		...
		v_item.l_inItem008 := pkcharacter.numeric_to_char(gOkCnt);	-- 件数
		v_item.l_inItem009 := '件';	-- "件"
		...
		v_item.l_inItem008 := pkcharacter.numeric_to_char(gSouCnt);	-- 総件数
		v_item.l_inItem009 := '件';	-- "件"
