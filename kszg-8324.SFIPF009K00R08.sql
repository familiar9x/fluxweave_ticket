...
CREATE OR REPLACE FUNCTION sfipf009k00r08 () RETURNS integer AS $body$
DECLARE
...
BEGIN	
	...
	cGyoumuDt := pkDate.getGyomuYmd();
        ...
        FROM  SOWN_INFO;
	-- 口座情報更新結果リストの呼び出し
	   CALL SPIPF009K00R01('IPF30000921', wk_kaiin, 'BATCH', '1', cGyoumuDt, rtn1, rtn2);
        -- 戻り値が'１'の場合
	IF  rtn1  =  1  THEN
            CALL pkLog.fatal(
           	     'ECM501',
        ...
        ELSIF  rtn1  IN (0)  THEN
            ...
            WHERE
                    ...
                    SC16.CHOHYO_ID = 'IPF30000921' AND
                    SC16.HEADER_FLG = 1;
            IF  ncount1  =  0  THEN
...
EXCEPTION
	WHEN OTHERS THEN
		-- ログ出力
		RAISE NOTICE 'FATAL Exception: % (%)', SQLERRM, SQLSTATE;
		CALL pkLog.fatal(
			'ECM701',
			'IPF009K00R08',
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION sfipf009k00r08 () FROM PUBLIC;
