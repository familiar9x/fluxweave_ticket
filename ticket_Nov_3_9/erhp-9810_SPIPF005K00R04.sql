...
BEGIN
	...
	IF coalesce(trim(both l_inItakuId)::text, '') = '' THEN
		...
		CALL pkLog.error('ECM501', 'IPF005K00R04', '＜項目名称:委託会社コード＞＜項目値:' || l_inItakuId || '＞');
		RETURN;
	END IF;
	-- 入力パラメータのチェック（データ種別）
	IF coalesce(trim(both l_inDataId)::text, '') = '' THEN
		l_outSqlCode := pkconstant.error();
	 	l_outSqlErrM := MSG_PARAM_ERROR;
		CALL pkLog.error('ECM501', 'IPF005K00R04',	'＜項目名称:データ種別＞＜項目値:' || l_inDataId || '＞');
		RETURN;
	END IF;
	-- 委託会社コードをチェック
	nCount := 0;
	...
	nCount := 0;
	SELECT count(*) INTO STRICT nCount FROM mbank_shiten_trns;
	...
	-- 金融機関支店マスタの件数をチェック
	nCount := 0;
	SELECT count(*) INTO STRICT nCount FROM mbank_shiten;
...
EXCEPTION
	WHEN OTHERS THEN
		CALL pkLog.fatal('ECM701', 'IPF005K00R04', 'SQLERRM:' || SQLERRM || '(' || SQLSTATE || ')');
