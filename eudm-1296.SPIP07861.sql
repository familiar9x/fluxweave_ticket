...
-- ***************************************************************************
	...
	--                  定数定義                                                    
	--==============================================================================
	C_PROCEDURE_ID  CONSTANT text := 'SPIP07861';                -- プロシージャＩＤ
	...
	-- 1 銘柄情報一覧
	C_SQL1	CONSTANT text	:=	'SELECT MG0.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,MG0.LAST_TEISEI_ID,MG0.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN '' '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MGR_STS MG0
										 ...
										 WHERE MG1.TOKUREI_SHASAI_FLG != ''Y''
										   ...
										   AND MG1.JTK_KBN !=''2''';
	-- 2 特例社債情報一覧
	C_SQL2	CONSTANT text	:=	'SELECT MG0.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,MG0.LAST_TEISEI_ID,MG0.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN '' '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MGR_STS MG0
										 ...
										 WHERE MG0.MASSHO_FLG = ''0''
										   ...
										   AND MG0.MGR_STAT_KBN IN (''0'')'; 
	-- 3 副受託銘柄情報一覧
	C_SQL3	CONSTANT text	:=	'SELECT MG0.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,MG0.LAST_TEISEI_ID,MG0.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN '' '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MGR_STS MG0
										  INNER JOIN MGR_KIHON MG1 ON MG0.ITAKU_KAISHA_CD = MG1.ITAKU_KAISHA_CD AND MG0.MGR_CD = MG1.MGR_CD
										  LEFT JOIN SGROUP SC08 ON MG0.GROUP_ID = SC08.GROUP_ID
										  LEFT JOIN VJIKO_ITAKU VJ ON MG0.ITAKU_KAISHA_CD = VJ.KAIIN_ID
										 ...
										 WHERE MG1.JTK_KBN = ''2''
										   ...
										   AND MG0.MGR_STAT_KBN IN(''0'')';
	-- 4 新規募集情報一覧
	C_SQL4	CONSTANT text	:=	'SELECT B01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,B01.LAST_TEISEI_ID,B01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN '' '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE TRIM(MG1.ISIN_CD) IS NOT NULL
										   ...
										   AND B01.SHORI_KBN =''0''';
	-- 5 新規記録情報承認可否一覧
	C_SQL5	CONSTANT text	:=	'SELECT B04a.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,B04a.LAST_TEISEI_ID,B04a.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MGR_STS MG0
											   INNER JOIN MGR_KIHON MG1 ON MG1.ITAKU_KAISHA_CD = MG0.ITAKU_KAISHA_CD AND MG1.MGR_CD = MG0.MGR_CD
											-- 承認対象となるメインテーブル
											INNER JOIN (SELECT
										...
										WHERE
											MG0.MASSHO_FLG = ''0''';
	-- 6 資金振替済確認
	C_SQL6	CONSTANT text	:=	'SELECT UA1.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,UA1.LAST_TEISEI_ID,UA1.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 7 決済指図データ一覧
	C_SQL7	CONSTANT text	:=	'SELECT B02.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,B02.LAST_TEISEI_ID,B02.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN,'' ''
	...
	-- 8 期中銘柄情報変更一覧
	C_SQL8	CONSTANT text	:=	'SELECT V.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,V.LAST_TEISEI_ID,V.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
											FROM
											    (SELECT
	...
	-- 9 元利金請求内容個別入力一覧
	C_SQL9	CONSTANT text	:=	'SELECT K07.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,K07.LAST_TEISEI_ID,K07.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM (SELECT K07.*,
													MG1.MGR_CD,
													MG1.MGR_RNM
	...
	-- 10機構非関与銘柄元利金請求データ
	C_SQL10	CONSTANT text	:=	'SELECT SK01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SK01.LAST_TEISEI_ID,SK01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM
											(SELECT
												K01.ITAKU_KAISHA_CD,
												K01.MGR_CD,
	...
	-- 11 基金入金確認
	C_SQL11	CONSTANT text	:=	'SELECT VK08.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,VK08.LAST_TEISEI_ID,VK08.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										FROM(
											SELECT
	...
	-- 12 現登債基金請求一覧
	C_SQL12	CONSTANT text	:=	'SELECT KK22.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,KK22.LAST_TEISEI_ID,KK22.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 13 差押設定／解除一覧
	C_SQL13	CONSTANT text	:=	'SELECT K04.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,K04.LAST_TEISEI_ID,K04.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM SASHIOSAE K04,
											   MGR_KIHON_VIEW VMG1,
											   SGROUP SC08,
											   VJIKO_ITAKU VJ,
	...
	-- 14 期中手数料補正一覧
	C_SQL14	CONSTANT text	:=	'SELECT VT01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,VT01.LAST_TEISEI_ID,VT01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM VTESURYO VT01
	...
	-- 15 会計区分別発行額データ一覧
	C_SQL15	CONSTANT text	:=	'SELECT SH02.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SH02.LAST_TEISEI_ID,SH02.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM (SELECT
													H02.ITAKU_KAISHA_CD,
													H02.MGR_CD,
													H02.GROUP_ID,
	...
	-- 16 グロスアップ銘柄税率データ一覧
	C_SQL16	CONSTANT text	:=	'SELECT X01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,X01.LAST_TEISEI_ID,VMG1.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 17 デフォルト情報一覧
	C_SQL17	CONSTANT text	:=	'SELECT DE01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,DE01.LAST_TEISEI_ID,DE01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	C_SQL18	CONSTANT text	:=	'SELECT X02.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,X02.LAST_TEISEI_ID,X02.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 19 銘柄情報（ＣＢ）一覧
	C_SQL19	CONSTANT text	:=	'SELECT MG0.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,MG0.LAST_TEISEI_ID,MG0.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MGR_STS MG0,
											   MGR_KIHON MG1,
											   CB_MGR_KIHON WMG1,
											   MHAKKOTAI M01,
	...
	-- 20期中銘柄情報（ＣＢ）変更一覧
	C_SQL20	CONSTANT text	:=	'SELECT V.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,V.LAST_TEISEI_ID,V.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM
										(SELECT
										''4'' AS KICHU_KBN,
									...
									AND V.LAST_TEISEI_ID = S1.USER_ID';
	-- 21 新規募集情報（ＣＢ）一覧
	C_SQL21	CONSTANT text	:=	'SELECT B01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,B01.LAST_TEISEI_ID,B01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM
											MGR_KIHON_VIEW VMG1,
											CB_MGR_KIHON   WMG1,
											SGROUP SC08,
	...
	C_SQL22	CONSTANT text	:=	'SELECT WK07.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,WK07.LAST_TEISEI_ID,WK07.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 23 新規記録結果情報（ＣＢ）一覧
	C_SQL23	CONSTANT text	:=	'SELECT WB05.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,WB05.LAST_TEISEI_ID,B01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM SHINKIBOSHU           B01,
	...
	-- 24 発行体マスタ
	C_SQL24	CONSTANT text	:=	'SELECT M01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M01.LAST_TEISEI_ID,M01.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM MHAKKOTAI M01,
											   SGROUP SC08,
											   VJIKO_ITAKU VJ,
											   (SELECT SC05.USER_ID USER_ID,MCD1.CODE_VALUE CODE_VALUE,MCD1.CODE_NM CODE_NM
	...
	-- 25金融機関マスタ
	C_SQL25	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M02.LAST_TEISEI_ID,M08.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 26 金融機関支店マスタ
	C_SQL26	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M03.LAST_TEISEI_ID,M10.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 27 部店マスタ
	C_SQL27	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M04.LAST_TEISEI_ID,M04.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 28 税務署マスタ
	C_SQL28	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M41.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE M41.SHORI_KBN IN (''0'',''2'')
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 29 税区分マスタ
	C_SQL29	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M42.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										   FROM
										  ...
										  WHERE M42.SHORI_KBN IN (''0'',''2'')
											AND VJ.JIKO_DAIKO_KBN = ''1''
											  ';
	-- 30 消費税マスタ
	C_SQL30	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M43.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE M43.SHORI_KBN IN (''0'',''2'')
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 31 国マスタ
	C_SQL31	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M63.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM
										...
										WHERE M63.SHORI_KBN IN (''0'',''2'')
										  AND VJ.JIKO_DAIKO_KBN = ''1''
										    ';
	-- 32 通貨マスタ
	C_SQL32	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M64.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE M64.SHORI_KBN IN (''0'',''2'')
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 33 元号マスタ
	C_SQL33	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,M65.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE M65.SHORI_KBN IN (''0'',''2'')
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 34 会計区分マスタ
	C_SQL34	CONSTANT text	:=	'SELECT W.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,W.LAST_TEISEI_ID,W.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
	...
	-- 35 ロールマスタ（画面）
	C_SQL35	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SC09.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE SC09.ROLE_ID LIKE ''1%''
										   ...
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 36 ロールマスタ（帳票）
	C_SQL36	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SC09.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
											FROM SROLE SC09,VJIKO_ITAKU VJ,
											   (SELECT SC05.USER_ID USER_ID,MCD1.CODE_VALUE CODE_VALUE,MCD1.CODE_NM CODE_NM
												  FROM SUSER SC05,SCODE MCD1
												 WHERE MCD1.CODE_SHUBETSU = ''902''
											...
											WHERE SC09.ROLE_ID LIKE ''2%''
											  ...
											  AND VJ.JIKO_DAIKO_KBN = ''1''
											    ';
	-- 37 ロールマスタ（代行）
	C_SQL37	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SC09.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										   FROM SROLE SC09,VJIKO_ITAKU VJ,
											   (SELECT SC05.USER_ID USER_ID,MCD1.CODE_VALUE CODE_VALUE,MCD1.CODE_NM CODE_NM
												  FROM SUSER SC05,SCODE MCD1
												 WHERE MCD1.CODE_SHUBETSU = ''902''
										  ...
										  WHERE SC09.ROLE_ID LIKE ''3%''
											...
											AND VJ.JIKO_DAIKO_KBN = ''1''
											  ';
	-- 38 グループマスタ
	C_SQL38	CONSTANT text	:=	'SELECT '' '' AS GROUP_ID,'' '' AS GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SC08.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										 ...
										 WHERE SC08.SHORI_KBN IN (''0'',''2'')
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 39 ユーザマスタ
	C_SQL39	CONSTANT text	:=	'SELECT SC05.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,SC05.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
										  FROM SUSER SC05,
											   SGROUP SC08,
											   VJIKO_ITAKU VJ,
											   (SELECT SC05.USER_ID USER_ID,MCD1.CODE_VALUE CODE_VALUE,MCD1.CODE_NM CODE_NM
										 ...
										 WHERE SC05.SHORI_KBN IN (''0'',''2'')
										   ...
										   AND VJ.JIKO_DAIKO_KBN = ''1''
										     ';
	-- 40 委託会社マスタ
	C_SQL40	CONSTANT text	:=	'SELECT JM01.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,JM01.LAST_TEISEI_ID,VJ.KAIIN_ID AS ITAKU_KAISHA_CD,'' '' AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' '' 
									     ...
									     WHERE JM01.SHORI_KBN IN (''0'',''2'')
									       ...
									       AND VJ.JIKO_DAIKO_KBN = ''1''
									         ';
	-- 41 銘柄情報登録テンプレートマスタ
	C_SQL41	CONSTANT text	:=	'SELECT X03.GROUP_ID,SC08.GROUP_NM,S1.CODE_VALUE,S1.CODE_NM,X03.LAST_TEISEI_ID,X03.ITAKU_KAISHA_CD,CASE WHEN VJ.JIKO_DAIKO_KBN=''1'' THEN ''  '' ELSE VJ.BANK_RNM END AS BANK_RNM,VJ.JIKO_DAIKO_KBN, '' ''
	...
	C_SQLSORT CONSTANT text	:= 'SELECT ITEM006, ITEM007, ITEM008, ITEM009, ITEM010, ITEM016, ITEM001, ITEM015, ITEM005 
											 FROM SREPORT_WK
...
BEGIN
    ...
    CALL SPIP07861_insertData(C_GAMEN_IDDUMMY, C_SQLSORT, C_CHOHYO_ID, '1', l_inItakuKaishaCd, l_inUserId, l_inChohyoKbn, l_inGyomuYmd, gRecCnt, gSeqNo, rec, gSakuseiYmd);
     -- ダミーデータ削除処理
     gresult := SPIP07861_deleteDummy(l_inItakuKaishaCd, l_inUserId, l_inChohyoKbn, l_inGyomuYmd, WK_CHOHYO_ID);
    IF gSeqNo = 1 THEN
        -- ヘッダレコードを追加
        ...
        BEGIN
            ...
            v_item.l_inItem011 := l_inUserId;               -- ユーザＩＤ
            v_item.l_inItem012 := C_CHOHYO_ID;              -- 帳票ＩＤ
            v_item.l_inItem013 := '対象データなし';         -- 対象データなし
...
CREATE OR REPLACE FUNCTION spip07861_deletedummy (
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION spip07861_deletedummy (l_inItakuKaishaCd text, l_inUserId text, l_inChohyoKbn char, l_inGyomuYmd char, WK_CHOHYO_ID text) FROM PUBLIC;
...
BEGIN
	...
	LOOP
        ...
        BEGIN
            -- Clear toàn bộ item
            v_item := ROW();
            v_item.l_inItem001 := rec[gRecCnt].gItakuKaishaRnm;          -- 委託会社略名
            v_item.l_inItem002 := l_inGyomuYmd;             -- 業務日付
            v_item.l_inItem003 := LPAD(pkcharacter.numeric_to_char(gSeqNo), 3, '0'); -- №
            v_item.l_inItem004 := l_inGamenSort;       		-- 画面ソート順
            v_item.l_inItem005 := gamenNm;        			-- 画面名称
            v_item.l_inItem006 := rec[gRecCnt].gGroupID;    -- グループID
            v_item.l_inItem007 := rec[gRecCnt].gGroupNm;    -- グループ名称
            v_item.l_inItem008 := rec[gRecCnt].gUserKngn;   -- ユーザー権限
            v_item.l_inItem009 := rec[gRecCnt].gUserKngnNm; -- ユーザー権限名称
            v_item.l_inItem010 := rec[gRecCnt].gLastUserId; -- 最終訂正者ID
            v_item.l_inItem011 := l_inUserId;              	-- ユーザーID
            v_item.l_inItem012 := l_inCHOHYOId;             -- 帳票ＩＤ
            v_item.l_inItem014 := gSakuseiYmd;              -- 作成日
            v_item.l_inItem015 := rec[gRecCnt].gJikoDaikoKbn;           -- 自行代行区分
            v_item.l_inItem016 := rec[gRecCnt].gItakuKaishaCd;        -- 委託会社コード
