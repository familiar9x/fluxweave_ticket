...
CREATE OR REPLACE PROCEDURE spipi046k15r01 ( l_ReportId text,                       --帳票ID
...
-- * 会社名: JIP
-- *
-- * 元利払請求書（領収書）【単票】を作成する。（バッチ用）
-- * １．請求データ検索処理
-- * ２．元利払請求書（領収書）【単票】作表処理
-- * ３．バッチ帳票出力ＯＮ処理
-- *
-- * @author Y.Yamada
...
BEGIN
	-- 入力パラメータのチェック
	IF coalesce(trim(both l_ReportId)::text, '') = '' OR coalesce(trim(both l_inItakuKaishaCd)::text, '') = '' OR coalesce(trim(both l_inItakuKaishaRnm)::text, '') = '' OR
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE spipi046k15r01 ( l_ReportId text, l_inItakuKaishaCd MITAKU_KAISHA.ITAKU_KAISHA_CD%TYPE, l_inItakuKaishaRnm SOWN_INFO.BANK_RNM%TYPE, l_inJikodaikoKbn text, l_inUserId SUSER.USER_ID%TYPE, l_inChohyoKbn text, l_inGyomuYmd TEXT, l_outSqlCode OUT numeric, l_outSqlErrM OUT text ) FROM PUBLIC;