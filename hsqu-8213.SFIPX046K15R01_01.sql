...
CREATE OR REPLACE FUNCTION sfipx046k15r01_01 ( l_initakuKaishaCd char(4) ) RETURNS integer AS $body$
DECLARE
...
--==============================================================================
	...
	wkChohyoId2			varchar(11);		-- ワーク帳票ＩＤ２
	...
	temp_rec sfipx046k15r01_01_type_record;
--==============================================================================
--					メイン処理													
--==============================================================================
BEGIN
  ...
  -- カウントの初期化
	gSeqNo := 0;
	rec := ARRAY[]::sfipx046k15r01_01_type_record[];
	-- カーソルオープン
  ...
  AND CHOHYO_ID = wkChohyoId2;
	...
	FOR gSeqNo IN 1..gMaxSeqNo LOOP
		...
		CALL pkPrint.insertData(
			...
			,l_inItem002	=>	rec[gSeqNo].RBR_KJT 		-- 利払期日
			,l_inItem003	=>	rec[gSeqNo].CHOKYU_YMD 		-- 徴求日
		...
		CALL pkPrint.insertData(
			...
			,l_inChohyoKbn	=>	'1'				-- 帳票区分
			,l_inSakuseiYmd	=>	gGyomuYmd 			-- 作成年月日
			,l_inChohyoId	=>	wkChohyoId2			-- WK帳票ＩＤ
			,l_inSeqNo	=>	gSeqNo 				-- SEQNO
			,l_inHeaderFlg	=>	'1'				-- ヘッダフラグ								-- 連番
			,l_inItem001	=>	rec[gSeqNo].MGR_CD 		-- 銘柄コード
			,l_inItem002	=>	rec[gSeqNo].RBR_KJT 		-- 利払期日
  ...
  IF gOptionFlg = 1 THEN
         ...
         gSqlErrm := SFIPX046K15R01_01_insKknIdo(  pkconstant.BATCH_USER(), gKjtFrom, gKjtTo, l_inItakuKaishaCd, '', '', '', '', '', gREPORT_ID, '1', '1', '');
...
DECLARE
  ...
  --                  タイプ                                                      
  --==============================================================================
    TYPE SFIPX046K15R01_01_insKknIdo_T_CURSOR  -- パラメータエラー
  --==============================================================================
  --                  タイプ                                                      
  --==============================================================================
    TYPE T_CURSOR REFCURSOR; -- カーソル・タイプ
 -- データ取得用タイプ
    recKknIdo SFIPX046K15R01_01_insKknIdo_TYPE_RECORD;
  --==============================================================================
  --                  変数定義                                                    
  --==============================================================================
    pCursor SFIPX046K15R01_01_insKknIdo_T_CURSOR; -- カーソル
  ...
  IF l_inChohyoKbn = '1' THEN
    ...
    SELECT coalesce(MAX(SC16.SEQ_NO), 0)
     ...
     WHERE SC16.KEY_CD = l_inItakuKaishaCd
       AND SC16.CHOHYO_ID = 'WK' || substring(l_inChohyoId, 3, 9);
  END IF;
  -- カーソルオープン
  CALL SFIPX046K15R01_01_insKknIdo_openCursor();
  -- ループ処理
  LOOP
    FETCH pCursor
      INTO recKknIdo;
    ...
    IF l_inChohyoKbn = '1' THEN
      ...
      CALL pkPrint.insertData(l_inKeyCd      => l_inItakuKaishaCd, -- 識別コード
                         ...
                         l_inSakuseiYmd => pGyomuYmd, -- 作成年月日
                         l_inChohyoId   => 'WK' ||
                                           substring(l_inChohyoId, 3, 9), -- 帳票ＩＤ
                         l_inSeqNo      => pSeqNo, -- 連番
                         l_inHeaderFlg  => 1, -- ヘッダーフラグ
                         l_inItem001    => recKknIdo.MGR_CD, -- アイテム１
                         l_inItem002    => recKknIdo.KJT, -- アイテム２
                         l_inItem003    => recKknIdo.CHOKYU_YMD, -- アイテム３
