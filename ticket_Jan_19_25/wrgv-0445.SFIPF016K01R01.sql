...
CREATE OR REPLACE FUNCTION sfipf016k01r01 () RETURNS integer AS $body$
DECLARE
...
--==============================================================================
	cGyoumuDt text;		--業務日付
	nCounter	numeric; 						--カウンター
	nRet		integer; 						--戻り値
	vItemRecord varchar(300);					--連結文字列格納
	cItem1	char(2)  := 'A1';					-- IF区分
	cItem2	char(2)  := '02';					-- データ区分
	cItem3	char(2)  := 'R2';					-- 業務区分
	cItem4	text := '                ';		-- IF通番
	cItem5	char(16) := '                ';		-- 当預約定番号
	cItem6	char(2)  := '  ';					-- 当預約定詳細番号
	cItem7	char(1)  := ' ';					-- データ処理区分
...
BEGIN
	...
	IF nRet != 0 THEN
		CALL pkLog.fatal('ECM701', 'IPF016K01R01', 'IF通番取得エラー');
		RETURN nRet;
	END IF;
	--更新日
...
EXCEPTION
	WHEN OTHERS THEN
		CALL pkLog.fatal('ECM701', 'IPF016K01R01', 'SQLERRM:' || SQLERRM || '(' || SQLSTATE || ')');
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION sfipf016k01r01 () FROM PUBLIC;
