...
--					定数定義													
--==============================================================================
	MSGID_CHECK_HANKAKU_EISU	CONSTANT text	:= 'ECM039'	;	--半角英数チェックエラー
	MSGID_CHECK_ZENKAKU 		CONSTANT text	:= 'ECM009'	;	--全角チェックエラー
	MSGID_CHECK_CODE			CONSTANT text	:= 'ECM305'	;	--コード値チェックエラー
	MSGID_CHECK_NUMBER			CONSTANT text	:= 'ECM002'	;	--数値チェックエラー
	MSGID_CHECK_ZENKAKU_KANA  	CONSTANT text	:= 'ECM058'	;	--全角カナチェックエラー
	MSGID_CHECK_KETASU			CONSTANT text	:= 'ECM012'	;	--桁数チェックエラー
	MSGID_CHECK_HANKAKU			CONSTANT text	:= 'ECM010'	;	--半角チェックエラー
	MSGID_CHECK_HIDUKE			CONSTANT text  	:= 'ECM005'	;	--日付チェックエラー
	MSGID_CHECK_HIKIOTOSHI		CONSTANT text	:= 'ECM001'	;	--引落しチェックエラー
	MSGID_CHECK_FUKUSURAN		CONSTANT text	:= 'ECM019'	;	--複数欄チェックエラー
	MSG_PARAM_ERROR CONSTANT text   := 'パラメーターエラー';
	MSG_NO_DATA     CONSTANT text   := 'データ無しエラー';
	MSG_DATA_ERROR  CONSTANT text   := 'データエラー';
	MSG_COMMON_ERR	CONSTANT text	:= '共通関数エラー'		;
--==============================================================================
--                  変数定義                                                    
--==============================================================================
...
BEGIN
		...
		IF nCount < 2 THEN
			...
			For iCnt IN 1 .. 32 LOOP
				CASE iCnt
					...
					-- 当預金額
					WHEN 22 THEN
						vData := vData || ',' || LPAD(pkcharacter.numeric_to_char(recToyoData.toyo_kngk), 18, '0');
					-- ＤＶＰ区分（ＳＭＢＣ）
					WHEN 23 THEN
	...
	VALUES(
		...
		1,
		(nRenban + 1),
		('TRL' || cGyoumuDt || LPAD(pkcharacter.numeric_to_char(nRowNum+1), 8, '0')),
		'0'
	);
	-- ファイル送受信保存テーブル
	INSERT INTO filesave(
